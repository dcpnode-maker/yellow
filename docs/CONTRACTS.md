...
`PartyProfileService.update` updates only the display and legal name of one active
same-tenant Party. It uses `profiles.party.update` idempotency and the governed
`update_party_profile` database command; the runtime role remains denied direct
`UPDATE` on `party`. The command requires an exact property/actor/request envelope,
transaction-local tenant context, and compare-and-swap current Party presentation.
It emits one minimized `party.updated` fact/outbox pair only when a value changes;
the pair carries Party identity and field names, never name values. An omitted legal
name preserves the existing value; explicit `null` clears it. Replays return the
stable original command receipt; a stale concurrent presentation fails closed.

`reconcile-synthetic-clean-arrival.ts` is an offline-only, target-bound repair, not
a generic Party attribute editor. It reads its compare-and-swap inputs through the
runtime RLS boundary and invokes one command limited to the canonical synthetic
`ARR-CLEAN` Party, its reservation and the reviewed `{source: local-review,
checkin_example: clean}` attributes. A changed repair emits minimized
`party.reconciled` evidence; a canonical rerun is a no-op.

`reconcile_synthetic_clean_arrival_account` is likewise offline-only and bound to
the one synthetic `ARR-CLEAN` guest account and primary folio. It locks the shared
Party/property/actor chain in the established order, verifies the reservation and
open primary folio, rejects posting or payment activity, updates only the canonical
account name under compare-and-swap, and emits minimized `account.reconciled`
fact/outbox evidence. It is not a generic account, folio, balance or payment editor.
...
identity documents, consent/preferences and contact/role editing remain planned.
...
`POST /api/v1/properties/{property}/folio-series` is the sole runtime configuration
boundary for the property's non-fiscal folio-number root. It requires the dedicated
`financials.folio-series:configure` permission over the exact property, JSON body
exactly `{prefix}`, and an `Idempotency-Key` of 8–200 visible ASCII characters.
Prefix is a trimmed 1–24 character value matching `[A-Za-z0-9/-]+`; tenant, actor,
property, correlation and property-local business date are server-owned.

First creation returns 201 and exactly
`{seriesId,tenantId,propertyNode,kind:'folio',prefix,fiscal:false,nextNo:'1',created:true}`.
Same-key replay preserves the original status/body and sets
`Idempotency-Replayed:true`. A new key with the exact same prefix returns 200 with the
current counter and `created:false`; it never resets a used counter. Different prefix,
any fiscal-shaped `kind='folio'` root, multiple folio roots, stale authority or
concurrent conflict fails closed. Before every idempotency lookup—including an exact
same-key replay—the service invokes the dedicated locked
`assert_non_fiscal_folio_series_configuration_authority(uuid,uuid,uuid)` capability.
The actor, tenant, property membership and permission must therefore still be active;
a historical receipt never revives revoked authority. The
command writes only one non-fiscal `document_series`, one minimized configured fact,
one `folio.series.configured` event and the idempotency receipt. It cannot configure
fiscal documents, allocate a folio number, open an account/folio, or mutate money,
reservation, occupancy, guest, room, tax or document state. Raw runtime series DML
remains revoked.
...
inputs before entering `Database.withTenantTransaction`. The SQL capabilities
recheck tenant, property, actor, permissions, document and provider-extension
identity from current database truth. A false Result or invalid receipt raises
a private transaction abort, then becomes a sanitized typed failure only after
rollback. A commit failure never returns success.

Canonical78 preserves the original submission head and adds one append-only,
FORCE-RLS receipt history. Request/retry execute only through the tenant app role;
claim/reconcile execute only through the dedicated runtime path. Owner-only
helpers derive exact immutable issued bytes and wire identity. Unknown delivery
requires lookup; only a proven known-not-sent result permits an explicit bounded
retry. No invoice, journal or number is regenerated by submission.

These are application command boundaries, not HTTP authorization or provider
certification. Q201 does not activate a server route, worker or external adapter.
Any later edge must bind verified session tenant/actor/property and a registered
adapter before effects. Exact
[durability proof](../tests/fiscal-submission-durability.integration.test.ts) and
...
## Governed property identity profile — Order581

`GET /api/v1/properties/{property}/profile` requires signed
`identity.property-profile:read`, an active actor and an ancestor grant covering the
exact property. Success is no-store JSON
`{property:{id,name,timezone,currency,version,effectiveAt,effectiveBusinessDate}}`.
Currency, effective instant and effective business date are nullable; version zero
means no governed rename fact exists and both effective fields are null. Raw
`org_node.config`, hierarchy paths and other property data are never returned.

`POST /api/v1/properties/{property}/profile/name` requires signed
`identity.property-profile:write`, the same live database authority, a visible-ASCII
`Idempotency-Key`, and body exactly `{expectedVersion,name}`. The server NFKC
normalizes, trims and collapses whitespace; the result must contain 1–200 visible
characters and no control, zero-width or bidi-format characters. A stale version is
409. An unchanged normalized name at the current version is a successful no-op with
no version/evidence advance. A real change returns
`{property:{...},changed:true,replayed:false}`; exact replay returns the stored
successful body with `replayed:true` after rechecking current authority. Reusing the
key with changed actor/property/body is 409.

The deployment-owned capability locks the exact property and changes only
`org_node.name`. Runtime has no direct name-update grant. Each changed commit advances
the append-only name version once, supersedes the preceding rename fact, derives its
instant and business date from PostgreSQL and the locked property timezone, and
atomically publishes `property.identity.changed`. Timezone, currency, path, kind and
the full config bytes are immutable in this contract. It adds no property creation,
address/content/amenity/meal/OTA/company/user configuration or check-in/out policy.

## Order593 — departure service coordination

`DepartureServiceCoordinationService` is the shared tenant-transaction command boundary.
Its capability owns persisted expiring proposals, canonical task creation, adjacent task
transitions and minimized transactional audit. It never posts charges, changes checkout,
occupancy, room condition or guest liability. New requests require one coherent occupied
room on an `in_house`/`due_out` reservation. Confirmed tasks may finish after checkout.

Routes (all no-store, exact property grants; all POSTs require Idempotency-Key):

- GET `/api/v1/properties/:property/reservations/:reservation/departure-services`:
  reservation identity/status, timezone, exact `{segmentId,spaceId,departureAt}` evidence
...
- POST reservation `/departure-services/proposals`: exact body
  ...
  Schedule is `{mode,minutes,localAt,utcOffsetMinutes}`: immediate has all nullable
  fields null; delay accepts10/15/30/45 minutes; custom accepts a property-local
  `YYYY-MM-DDTHH:mm[:ss]` plus explicit minutes east of UTC. PostgreSQL round-trips the
  instant against the property timezone, rejecting nonexistent or wrong-offset times.
  A proposal expires after10 minutes and creates no operational task.
- POST property `/departure-services/:request/:action`, or the reservation-scoped alias:
  actions `confirm|withdraw|assign|start|complete`; exact body
  `{expectedVersion,staffPartyId,outcome}`. Staff is non-null only for assign. Outcome
  is non-null only for completed minibar/room checks and must be
  `clear|finding_reported|unable_to_complete`. Luggage/escalation completion takes null.

Commands return `{request,replayed}`. Request includes exact request/reservation/segment/
room/role IDs, service, parent, proposal status/version/expiry, departure/due instants,
property-local due display, timezone, task identity/status/assignee/completion, bounded
outcome and granted next actions. It includes no guest contact, allegation or amount.
Confirmation binds the persisted version and revalidates its exact stay evidence;
immediate due time is server transaction time. Every request must be within six hours
after its recorded departure; scheduled instants must still be future at confirmation.

Exact scopes are `stay-operations.departure-services:read|request|confirm|dispatch|work|escalate`.
Both token scope and current property grant are necessary; actorless clients cannot act.
Routing roles are only Front Desk Cashier, Housekeeping Desk, Housekeeping Team Lead,
Assistant Manager and Duty Manager, with an active colleague's exact-property role
membership and read permission. Task workers require separate work authority. Staff
Party selection does not invent a Party/app_user identity relationship.

Durable uniqueness admits one non-withdrawn root request per reservation/segment/service
and one non-withdrawn escalation per parent/role. Expired proposals remain visible and
