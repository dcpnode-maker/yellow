...
COPY --chown=bun:bun package.json bun.lock ./
COPY --chown=bun:bun src ./src
# Yellow Next is selected by the public route at runtime. Keep only the
# pre-built static surface; source maps, environment files and runtime output
# remain outside the image context.
COPY --chown=bun:bun public/yellow-next ./public/yellow-next
