# Order 335 — Room outages destination-copy local refresh

**Status:** READY-D936
**Phase:** 7 — founder-visible presentation of already-built journeys
**Branch:** `phase-7/room-outages-destination-copy-local-refresh`
**Base:** `fc5b048` (independently approved Order334 governance head)
**Runtime source:** `15516170433b008411bb07e13c8001f823f8e16d`
**Risk tier:** 3 — sole founder-local replacement; fresh non-operating review mandatory

## Outcome

Reflect approved Order334 in the one stable loopback3000 management-demo app.

## Exact scope

- build exact approved runtime and replace only the sole app;
- retain Order333 stopped for rollback and inherit exact environment/network/bind/health;
- verify protected login,two properties,24 routes,status,live Room outages copy and
  all previously approved presentation contracts;
- prove database,companions,data and obsolete ports unchanged;
- fresh non-operating Tier3 read-only review.

## Forbidden

No second/public local,schema,migration,seed,database/data,credential,status,
permission,authority or post310 statutory change;no merge,push,deployment or rollback deletion.

## Definition of done

- [ ] Exact image is sole healthy loopback3000 and Order333 is retained stopped.
- [ ] Protected/browser/routes/status/database/topology proof passes.
- [ ] Fresh non-operating Tier3 reviewer approves exact runtime.

