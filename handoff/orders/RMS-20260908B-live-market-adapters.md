...
Record observed public probe requests, times, counts and failures outside public git; only synthetic fixtures enter tests. Test money parsing without float arithmetic, stale/unknown freshness, request/response mismatch, URL and credential redaction, bounded size/time/status handling, no retry on access blocks, source/horizon opt-in, month-end/LOS boundaries, duplicate grouping, inactive configuration and private output refusal. Independently execute final focused proof and real captured-response ingestion. No claimed live Google result without an actual successful call/capture.
