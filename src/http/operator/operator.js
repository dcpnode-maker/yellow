(() => {
 ...
 let guestProfileRequest = null;
 let guestProfileReturnFocus = null;
 ...
 let cashierBillingSearchGeneration = 0;
 ...
 const clientWebsitePreview = $("#client-website-preview");
 ...
 const todayBoundary = $("#today-boundary");
 ...
 const cashierBillingSearchForm = $("#cashier-billing-search-form");
 const cashierBillingQuery = $("#cashier-billing-query");
 const cashierBillingMessage = $("#cashier-billing-message");
 const cashierBillingResults = $("#cashier-billing-results");
 ...
 const rateBuilder = $("#rate-builder");
 ...
 const guestProfileDrawer = $("#guest-profile-drawer");
 const guestProfileTitle = $("#guest-profile-title");
 const guestProfileClose = $("#guest-profile-close");
 const guestProfileLoading = $("#guest-profile-loading");
 const guestProfileError = $("#guest-profile-error");
 const guestProfileRetry = $("#guest-profile-retry");
 const guestProfileContent = $("#guest-profile-content");
 const guestProfileStatus = $("#guest-profile-status");
 ...
 if (body.properties.length === 0) {
  ...
  if (pathProperty && body.properties.some(({ id }) => id === pathProperty)) {
   propertySelect.value = pathProperty;
  } else {
   // Start a public review on the coherent operational scenario: it contains
   // arrivals, a departure and an in-house stay. Riverstone remains available in
   // the switcher as the richer commercial-configuration fixture.
   const operationalDemo = body.properties.find(({ name }) => name === "Yellow Demo Property")
     ?? body.properties.find(({ name }) => name === "Riverstone Test Hotel");
   if (operationalDemo) propertySelect.value = operationalDemo.id;
  }
 }
 }
 function syncClientWebsitePreview() {
 const property = propertiesData.find(({ id }) => id === propertySelect.value);
 const locandaContext = property && /locanda|jareed/i.test(String(property.name || ""));
 clientWebsitePreview.hidden = !property;
 clientWebsitePreview.textContent = locandaContext ? "Preview guest website" : "Open Locanda Homes preview";
 clientWebsitePreview.setAttribute("aria-label", locandaContext
  ? "Preview this property’s guest website"
  : "Open the attached Locanda Homes guest website preview");
 }
 ...
 function reservationPeriod(from, to) {
  const start = new Date(String(from));
  const end = new Date(String(to));
  const publicDemo = document.documentElement.dataset.yellowAutomaticDemoLogin === "1";
  const engineeringRange = Number.isFinite(start.getTime()) && Number.isFinite(end.getTime())
   && start.getUTCFullYear() <= 2021 && end.getUTCFullYear() >= 2099;
  if (publicDemo && engineeringRange) {
   const today = new Date(); today.setUTCHours(0, 0, 0, 0);
   const tomorrow = new Date(today); tomorrow.setUTCDate(tomorrow.getUTCDate() + 1);
   const day = new Intl.DateTimeFormat(undefined, { dateStyle: "medium", timeZone: "UTC" });
   return `Synthetic demo operational window · ${day.format(today)} 3:00 PM – ${day.format(tomorrow)} 11:00 AM UTC`;
  }
  return `${reservationDateTime(from)} – ${reservationDateTime(to)}`;
 }
 function reservationStay(row) {
 return reservationPeriod(row.stayFrom, row.stayTo);
 }
...
const RESERVATION_TRAVEL_MODE_LABELS = Object.freeze({
 ...
 button.addEventListener("click", () => void openReservationDetail(row.reservationId, { trigger: button }));
  ...
  function reservationGuestProfileButton(row) {
   const button = el("button");
   button.type = "button";
   button.className = "reservation-guest-profile-open";
   button.textContent = row.primaryGuestDisplayName;
   button.setAttribute("aria-label", `Open guest and stay profile for ${row.primaryGuestDisplayName}`);
   button.addEventListener("click", () => void openGuestProfile(row, { trigger: button }));
   return button;
  }
  function guestProfileSection(title, entries) {
   const section = node("section", "guest-profile-section");
  ...
  function renderGuestProfile(profile, history) {
   guestProfileTitle.textContent = profile.displayName;
   guestProfileContent.replaceChildren();
   const contacts = Array.isArray(profile.contacts) && profile.contacts.length
    ? profile.contacts.map((contact) => `${contact.kind}${contact.isPrimary ? " (primary)" : ""}: ${contact.hint}`).join(" · ")
    : "No contact hints recorded";
   guestProfileContent.append(guestProfileSection("Guest record", [
    ["Party type", profile.kind], ["Roles", Array.isArray(profile.roles) ? profile.roles.join(", ") || "Not recorded" : "Not recorded"],
    ["Contacts", contacts],
   ]));
   const stays = node("section", "guest-profile-section");
   stays.append(node("h4", "", "Stay history"));
   const historyList = node("div", "guest-stay-history");
   if (!Array.isArray(history) || history.length === 0) {
    historyList.append(node("p", "muted", "No stays are recorded for this guest at the selected property."));
   } else {
    for (const stay of history) {
     const action = node("button", "quiet", `${stay.confirmationNo} · ${reservationPeriod(stay.stayFrom, stay.stayTo)} · ${reservationStatusLabels[stay.status] || stay.status}`);
     action.type = "button";
     action.setAttribute("aria-label", `Open reservation ${stay.confirmationNo} from ${profile.displayName}'s stay history`);
     action.addEventListener("click", () => {
      closeGuestProfile({ restoreFocus: false });
      void openReservationDetail(stay.reservationId, { trigger: action });
     });
     historyList.append(action);
    }
   }
   stays.append(historyList);
   guestProfileContent.append(stays);
   guestProfileLoading.hidden = true;
   guestProfileError.hidden = true;
   guestProfileContent.hidden = false;
   guestProfileDrawer.setAttribute("aria-busy", "false");
   guestProfileStatus.textContent = "Party profile and property-scoped stay history loaded from server truth.";
  }
  function closeGuestProfile({ restoreFocus = true } = {}) {
   if (guestProfileDrawer.hidden) return;
   const focus = guestProfileReturnFocus;
   guestProfileDrawer.hidden = true;
   guestProfileDrawer.setAttribute("aria-busy", "false");
   guestProfileContent.replaceChildren();
   guestProfileRequest = null;
   guestProfileReturnFocus = null;
   if (restoreFocus && focus?.isConnected) focus.focus({ preventScroll: true });
  }
  async function openGuestProfile(row, { trigger = null } = {}) {
   closeReservationDetail({ history: false, restoreFocus: false });
   guestProfileReturnFocus = trigger;
   guestProfileRequest = Object.freeze({ partyId: canonicalUuid(row.primaryPartyId) ? row.primaryPartyId : "", property: propertySelect.value, reservationId: row.reservationId || "" });
   const generation = ++guestProfileGeneration;
   guestProfileDrawer.hidden = false;
   guestProfileDrawer.setAttribute("aria-busy", "true");
   guestProfileTitle.textContent = "Loading guest…";
   guestProfileLoading.hidden = false;
   guestProfileError.hidden = true;
   guestProfileContent.hidden = true;
   guestProfileStatus.textContent = "Loading Party profile and stay history…";
   guestProfileDrawer.focus({ preventScroll: true });
   try {
    const property = propertySelect.value;
    let partyId = row.primaryPartyId;
    if (!canonicalUuid(partyId) && canonicalUuid(row.reservationId)) {
     const detail = await request(`/api/v1/properties/${enc(property)}/reservations/${enc(row.reservationId)}`);
     partyId = detail?.reservation?.primaryPartyId;
    }
    if (!canonicalUuid(partyId)) throw new Error("The selected stay does not have a valid primary Party record.");
    guestProfileRequest = Object.freeze({ partyId, property, reservationId: row.reservationId || "" });
    const [profiles, board] = await Promise.all([
     request(`/api/v1/properties/${enc(property)}/parties:search`, { method: "POST", body: JSON.stringify({ query: partyId, limit: 1 }) }),
     request(`/api/v1/properties/${enc(property)}/reservation-board?${new URLSearchParams({ partyId, limit: "50" })}`),
    ]);
    if (generation !== guestProfileGeneration || guestProfileRequest?.partyId !== partyId || property !== propertySelect.value) return;
    const profile = Array.isArray(profiles?.profiles) ? profiles.profiles.find((item) => item.partyId === partyId) : null;
    if (!profile) throw new Error("The guest Party profile is unavailable for this property.");
    renderGuestProfile(profile, Array.isArray(board?.reservations) ? board.reservations : []);
   } catch (error) {
    if (generation !== guestProfileGeneration) return;
    guestProfileLoading.hidden = true;
    guestProfileError.hidden = false;
    guestProfileError.querySelector("p").textContent = error?.status === 403
     ? "Guest profile access is not granted for this property."
     : error instanceof Error ? error.message : "The guest profile is unavailable.";
    guestProfileDrawer.setAttribute("aria-busy", "false");
    guestProfileStatus.textContent = "Guest profile unavailable.";
   }
  }
  function reservationTableRow(row) {
   const tr = el("tr");
   const values = [
   reservationOpenButton(row), reservationGuestProfileButton(row), reservationStatusBadge(row.status),
 ...
 const guest = reservationGuestProfileButton(row);
 guest.classList.add("reservation-card-guest-profile");
...
function checkoutHousekeepingCompletionActionIsCurrent(origin, section, action) {
 ...
 reservationDetailContent.append(
  detailSection("Guest & stay profile", [
  ["Guest", primary?.displayName || "Not recorded"],
  ["Party record", primary?.partyId || "Not recorded"],
  ...
  `${reservationPeriod(segment.from, segment.to)} · ${segment.adults} adult${segment.adults === 1 ? "" : "s"} · ${segment.status}`),
  ...
  detailCollection("Stay history", reservation.history, (fact) =>
 ...
 function cashierBillingMatches(row, query) {
 const needle = query.trim().toLocaleLowerCase();
 return needle.length > 0 && [row?.primaryGuestDisplayName, row?.confirmationNo]
  .some((value) => typeof value === "string" && value.toLocaleLowerCase().includes(needle));
 }
 function cashierBillingResultLabel(row) {
 const room = typeof row.sellableUnitLabel === "string" && row.sellableUnitLabel ? ` · ${row.sellableUnitLabel}` : " · Room pending";
 return `${row.primaryGuestDisplayName} · ${row.confirmationNo}${room}`;
 }
 function renderCashierBillingResults(rows) {
 cashierBillingResults.replaceChildren();
 for (const row of rows) {
 if (!row || !canonicalUuid(row.reservationId) || typeof row.primaryGuestDisplayName !== "string" || typeof row.confirmationNo !== "string") continue;
 const button = node("button", "secondary cashier-billing-result", cashierBillingResultLabel(row));
 button.type = "button";
 button.dataset.reservationId = row.reservationId;
 button.setAttribute("aria-label", `Open billing desk for ${row.primaryGuestDisplayName}, ${row.confirmationNo}`);
 button.addEventListener("click", () => void openCashierBillingFolio(row.reservationId, button));
 cashierBillingResults.append(button);
 }
 }
 async function openCashierBillingFolio(reservationId, control) {
 if (!canonicalUuid(reservationId) || activeView !== "cashiers") return;
 const property = propertySelect.value;
 control.disabled = true;
 cashierBillingMessage.classList.remove("error");
 cashierBillingMessage.textContent = "Resolving the stay’s existing server-owned folio windows…";
 try {
 const detail = await request(`/api/v1/properties/${enc(property)}/reservations/${enc(reservationId)}`);
 if (activeView !== "cashiers" || property !== propertySelect.value) return;
 const reservation = detail?.reservation;
 const folios = Array.isArray(reservation?.folios) ? reservation.folios : [];
 const primary = folios.find((folio) => folio?.windowNo === 1 && canonicalUuid(folio?.folioId)) || folios.find((folio) => canonicalUuid(folio?.folioId));
 if (!primary) { cashierBillingMessage.textContent = "This stay has no existing folio window. Open one only through the explicit reservation command."; return; }
 cashierBillingMessage.textContent = "Opening the selected guest’s immutable billing workspace…";
 history.pushState({ yellowSurface: "folio-workspace" }, "", canonicalFolioPath(property, primary.folioId));
 setView("folios", false);
 finishWorkspaceNavigation("folios");
 void loadFolioWorkspace(primary.folioId);
 } catch (error) {
 if (activeView === "cashiers" && property === propertySelect.value) { cashierBillingMessage.classList.add("error"); cashierBillingMessage.textContent = error instanceof Error ? error.message : "The selected stay could not be resolved."; }
 } finally { if (activeView === "cashiers" && property === propertySelect.value) control.disabled = false; }
 }
 async function searchCashierBillingDesk() {
 const query = cashierBillingQuery.value.trim();
 const property = propertySelect.value;
 if (query.length < 1) { cashierBillingResults.replaceChildren(); cashierBillingMessage.textContent = "Enter a guest name or confirmation."; cashierBillingQuery.focus(); return; }
 const generation = ++cashierBillingSearchGeneration;
 cashierBillingResults.replaceChildren(); cashierBillingMessage.classList.remove("error"); cashierBillingMessage.textContent = "Searching the current property’s in-house stays…";
 try {
 const page = await request(`/api/v1/properties/${enc(property)}/reservation-board?status=in_house&limit=100`);
 if (generation !== cashierBillingSearchGeneration || activeView !== "cashiers" || property !== propertySelect.value) return;
 const matches = (Array.isArray(page?.reservations) ? page.reservations : []).filter((row) => cashierBillingMatches(row, query)).slice(0, 25);
 renderCashierBillingResults(matches);
 cashierBillingMessage.textContent = matches.length === 0 ? "No current in-house stay matched. Search is restricted to this property’s active in-house records." : `${matches.length} current in-house stay${matches.length === 1 ? "" : "s"} matched. Select one to open its billing workspace.`;
 } catch (error) {
 if (generation !== cashierBillingSearchGeneration || activeView !== "cashiers" || property !== propertySelect.value) return;
 cashierBillingMessage.classList.add("error"); cashierBillingMessage.textContent = error instanceof Error ? error.message : "Current in-house stays could not be searched.";
 }
 }
...
function vehicleReturnPathFromState(state, property) {
 ...
 const period = node("span", "segment-meta", reservationPeriod(segment.period.from, segment.period.to));
 ...
 if (document.documentElement.dataset.yellowAutomaticDemoLogin === "1") {
  void (async () => {
   const button = loginForm.querySelector("button[type=submit]");
   button.disabled = true;
 ...
 async function searchReservationBookingOffers() {
 ...
 reservationBookingForm.addEventListener("submit", (event) => {
  event.preventDefault();
  void searchReservationBookingOffers();
 });
 ...
 if (event.key === "Escape" && !guestProfileDrawer.hidden) {
  event.preventDefault();
  closeGuestProfile();
  return;
 }
 ...
 guestProfileClose.addEventListener("click", () => closeGuestProfile());
 guestProfileRetry.addEventListener("click", () => {
  const retry = guestProfileRequest;
  if (!retry) return;
  void openGuestProfile({ primaryPartyId: retry.partyId }, { trigger: guestProfileReturnFocus });
 });
 todayRefresh.addEventListener("click", () => { delete todayView.dataset.jarvisFocus; todayBoundary.textContent = "Due-in, due-out and in-house reservations overlapping this property-local day. Each list is one bounded server page, not a hotel-wide total."; loadToday(); });
...
housekeepingSheetDate.addEventListener("change", () => {
 ...
 reservationSearchOffers.hidden = false;
 void searchReservationBookingOffers();
});
 reservationOfferRetry.addEventListener("click", () => {
  reservationSearchOffers.hidden = false;
  void searchReservationBookingOffers();
 });
 ...
 cashierBillingSearchForm.addEventListener("submit", (event) => { event.preventDefault(); void searchCashierBillingDesk(); });
 ...
 const hotelSetupJourney = document.querySelector(".hotel-setup-journey");
 const legacyRateTools = document.querySelector(".legacy-rate-tools");
 const openHotelRateStrategy = (step = 1) => {
  rateBuilder.hidden = false;
  setBuilderStep(step);
  rateBuilder.scrollIntoView({ behavior:"smooth", block:"start" });
  requestAnimationFrame(() => document.getElementById("rate-builder-title")?.focus({ preventScroll:true }));
 };
 $("#hotel-setup-open-rate-builder").addEventListener("click", () => openHotelRateStrategy(1));
 $("#hotel-setup-show-advanced").addEventListener("click", () => { legacyRateTools.open = true; openHotelRateStrategy(1); });
 hotelSetupJourney.addEventListener("click", event => {
  const viewButton = event.target.closest("[data-hotel-setup-view]");
  if (viewButton) { setView(viewButton.dataset.hotelSetupView); finishWorkspaceNavigation(viewButton.dataset.hotelSetupView); return; }
  const stepButton = event.target.closest("[data-hotel-setup-rate-step]");
  if (stepButton) openHotelRateStrategy(Number(stepButton.dataset.hotelSetupRateStep));
 });
 ...
 const jarvisLaunch = $("#jarvis-launch"), jarvisDialog = $("#jarvis-dialog"), jarvisForm = $("#jarvis-form"), jarvisMessage = $("#jarvis-message"), jarvisMessages = $("#jarvis-messages"), jarvisMic = $("#jarvis-mic"), jarvisPips = $("#jarvis-pips"), jarvisListening = $("#jarvis-listening"), jarvisLanguage = $("#jarvis-language"), jarvisVoiceStyle = $("#jarvis-voice-style"), jarvisVoiceStatus = $("#jarvis-voice-status"), jarvisActivity = $("#jarvis-activity-list"), jarvisClear = $("#jarvis-clear");
 const jarvisViews = Object.freeze({ today:"today", reservations:"reservations", housekeeping:"housekeeping", rates:"rates", folios:"folios", guests:"reservations" });
 const jarvisFocusLabels = Object.freeze({ due_in:"arrivals / due-ins", due_out:"departures / due-outs", in_house:"in-house stays" });
 // Keep this demo conversation only in the live page. An operator can clear it
 // explicitly, and a reload discards it instead of retaining a guest name or an
 // accidental sensitive prompt in browser storage.
 let jarvisHistory = [];
 let jarvisVoiceEnabled = false;
 let jarvisVoicePreference = { language:"en-IN", style:"india-young-male" };
 let overwatchLanguagePrompted = false;
 let overwatchLanguageSelected = false;
 if ([...jarvisLanguage.options].some(option => option.value === jarvisVoicePreference.language)) jarvisLanguage.value = jarvisVoicePreference.language;
 if ([...jarvisVoiceStyle.options].some(option => option.value === jarvisVoicePreference.style)) jarvisVoiceStyle.value = jarvisVoicePreference.style;
 function overwatchText(key, values = {}) {
  const language = jarvisVoicePreference.language;
  const copy = {
   "en-IN": {
    arrivals:"Showing today’s arrivals from the live PMS lane. Say “open [guest name]” to expand a reservation, or “prepare check-in [guest name]” to start the governed check-in flow.",
    departures:"Showing today’s departures from the live PMS lane.", inHouse:"Showing the current in-house lane from the live PMS.",
    reservations:"Opening reservations. Say a guest name to narrow the live list.", housekeeping:"Opening the live housekeeping workspace.", rates:"Opening rates and inventory.", folios:"Opening folios and invoices.",
    opened:"Opening {guest}’s live reservation.", preparing:"Opening {guest}’s live check-in preparation. I will show room, folio and readiness blockers before any change.",
    noMatch:"I cannot uniquely match “{name}” in the live arrivals, departures or in-house lists yet. Say “show today’s arrivals” first, then use the guest name exactly as shown.",
    selected:"Great. I’ll speak {language}. What would you like to do?", live:"Live {focus}", refresh:"Refreshing the property-local PMS lane…", liveNote:"Live PMS data · changes stay visible here while Overwatch keeps this conversation open.", dueIn:"arrivals / due-ins", dueOut:"departures / due-outs", inHouseLabel:"in-house stays"
   },
   "hi-IN": {
    arrivals:"लाइव PMS से आज के आगमन दिखा रहा हूँ। किसी आरक्षण को खोलने के लिए “open [guest name]” या नियंत्रित चेक-इन शुरू करने के लिए “prepare check-in [guest name]” कहें।",
    departures:"लाइव PMS से आज के प्रस्थान दिखा रहा हूँ।", inHouse:"लाइव PMS से वर्तमान इन-हाउस अतिथि दिखा रहा हूँ।",
    reservations:"आरक्षण खोल रहा हूँ। लाइव सूची को सीमित करने के लिए अतिथि का नाम कहें।", housekeeping:"लाइव हाउसकीपिंग कार्यक्षेत्र खोल रहा हूँ।", rates:"दरें और इन्वेंटरी खोल रहा हूँ।", folios:"फोलियो और इनवॉइस खोल रहा हूँ।",
    opened:"{guest} का लाइव आरक्षण खोल रहा हूँ।", preparing:"{guest} के लाइव चेक-इन की तैयारी खोल रहा हूँ। किसी भी बदलाव से पहले मैं कमरा, फोलियो और तैयारी में बाधाएँ दिखाऊँगा।",
    noMatch:"लाइव आगमन, प्रस्थान या इन-हाउस सूची में “{name}” का एक स्पष्ट मिलान नहीं मिला। पहले “show today’s arrivals” कहें, फिर नाम जैसा दिख रहा है वैसा ही बोलें।",
    selected:"ठीक है। मैं {language} में बात करूँगा। आप क्या करना चाहेंगे?", live:"लाइव {focus}", refresh:"प्रॉपर्टी-स्थानीय PMS सूची ताज़ा की जा रही है…", liveNote:"लाइव PMS डेटा · Overwatch बातचीत खुली रखते हुए बदलाव यहाँ दिखते रहेंगे।", dueIn:"आगमन / ड्यू-इन", dueOut:"प्रस्थान / ड्यू-आउट", inHouseLabel:"इन-हाउस स्टे"
   },
   "mr-IN": {
    arrivals:"लाइव्ह PMS मधून आजचे आगमन दाखवत आहे. आरक्षण उघडण्यासाठी “open [guest name]” किंवा नियंत्रित चेक-इन सुरू करण्यासाठी “prepare check-in [guest name]” म्हणा.",
    departures:"लाइव्ह PMS मधून आजचे निर्गमन दाखवत आहे.", inHouse:"लाइव्ह PMS मधून सध्या मुक्कामी असलेले अतिथी दाखवत आहे.",
    reservations:"आरक्षणे उघडत आहे. लाइव्ह यादी मर्यादित करण्यासाठी अतिथीचे नाव म्हणा.", housekeeping:"लाइव्ह हाउसकीपिंग कार्यक्षेत्र उघडत आहे.", rates:"दर आणि इन्व्हेंटरी उघडत आहे.", folios:"फोलिओ आणि इनव्हॉइस उघडत आहे.",
    opened:"{guest} यांचे लाइव्ह आरक्षण उघडत आहे.", preparing:"{guest} यांच्या लाइव्ह चेक-इनची तयारी उघडत आहे. कोणताही बदल करण्यापूर्वी मी खोली, फोलिओ आणि तयारीतील अडथळे दाखवीन.",
    noMatch:"लाइव्ह आगमन, निर्गमन किंवा मुक्कामी यादीमध्ये “{name}” यांचा एक स्पष्ट जुळवाजुळव सापडला नाही. आधी “show today’s arrivals” म्हणा, नंतर दिसत असलेले नाव तंतोतंत म्हणा.",
    selected:"छान. मी {language} मध्ये बोलेन. तुम्हाला काय करायचे आहे?", live:"लाइव्ह {focus}", refresh:"प्रॉपर्टी-स्थानिक PMS यादी रिफ्रेश करत आहे…", liveNote:"लाइव्ह PMS डेटा · Overwatch ही चर्चा सुरू ठेवताना बदल येथे दिसत राहतील.", dueIn:"आगमन / ड्यू-इन", dueOut:"निर्गमन / ड्यू-आउट", inHouseLabel:"मुक्कामी निवास"
   },
   "kn-IN": {
    arrivals:"ಲೈವ್ PMS ನಿಂದ ಇಂದಿನ ಆಗಮನಗಳನ್ನು ತೋರಿಸುತ್ತಿದ್ದೇನೆ. ಕಾಯ್ದಿರಿಸುವಿಕೆಯನ್ನು ತೆರೆಯಲು “open [guest name]” ಅಥವಾ ನಿಯಂತ್ರಿತ ಚೆಕ್-ಇನ್ ಆರಂಭಿಸಲು “prepare check-in [guest name]” ಎಂದು ಹೇಳಿ.",
    departures:"ಲೈವ್ PMS ನಿಂದ ಇಂದಿನ ನಿರ್ಗಮನಗಳನ್ನು ತೋರಿಸುತ್ತಿದ್ದೇನೆ.", inHouse:"ಲೈವ್ PMS ನಿಂದ ಪ್ರಸ್ತುತ ತಂಗಿರುವ ಅತಿಥಿಗಳನ್ನು ತೋರಿಸುತ್ತಿದ್ದೇನೆ.",
    reservations:"ಕಾಯ್ದಿರಿಸುವಿಕೆಗಳನ್ನು ತೆರೆಯುತ್ತಿದ್ದೇನೆ. ಲೈವ್ ಪಟ್ಟಿಯನ್ನು ಸೀಮಿತಗೊಳಿಸಲು ಅತಿಥಿಯ ಹೆಸರನ್ನು ಹೇಳಿ.", housekeeping:"ಲೈವ್ ಹೌಸ್‌ಕೀಪಿಂಗ್ ಕಾರ್ಯಕ್ಷೇತ್ರವನ್ನು ತೆರೆಯುತ್ತಿದ್ದೇನೆ.", rates:"ದರಗಳು ಮತ್ತು ಇನ್‌ವೆಂಟರಿಯನ್ನು ತೆರೆಯುತ್ತಿದ್ದೇನೆ.", folios:"ಫೋಲಿಯೊಗಳು ಮತ್ತು ಇನ್‌ವಾಯ್ಸ್‌ಗಳನ್ನು ತೆರೆಯುತ್ತಿದ್ದೇನೆ.",
    opened:"{guest} ಅವರ ಲೈವ್ ಕಾಯ್ದಿರಿಸುವಿಕೆಯನ್ನು ತೆರೆಯುತ್ತಿದ್ದೇನೆ.", preparing:"{guest} ಅವರ ಲೈವ್ ಚೆಕ್-ಇನ್ ಸಿದ್ಧತೆಯನ್ನು ತೆರೆಯುತ್ತಿದ್ದೇನೆ. ಯಾವುದೇ ಬದಲಾವಣೆಗೆ ಮೊದಲು ಕೊಠಡಿ, ಫೋಲಿಯೊ ಮತ್ತು ಸಿದ್ಧತೆಯ ಅಡಚಣೆಗಳನ್ನು ತೋರಿಸುತ್ತೇನೆ.",
    noMatch:"ಲೈವ್ ಆಗಮನ, ನಿರ್ಗಮನ ಅಥವಾ ತಂಗಿರುವವರ ಪಟ್ಟಿಯಲ್ಲಿ “{name}” ಗೆ ಒಂದು ಸ್ಪಷ್ಟ ಹೊಂದಾಣಿಕೆ ಸಿಗಲಿಲ್ಲ. ಮೊದಲು “show today’s arrivals” ಎಂದು ಹೇಳಿ, ನಂತರ ಕಾಣುವ ಹೆಸರನ್ನೇ ನಿಖರವಾಗಿ ಹೇಳಿ.",
    selected:"ಸರಿ. ನಾನು {language} ನಲ್ಲಿ ಮಾತನಾಡುತ್ತೇನೆ. ನಿಮಗೆ ಏನು ಬೇಕು?", live:"ಲೈವ್ {focus}", refresh:"ಆಸ್ತಿ-ಸ್ಥಳೀಯ PMS ಪಟ್ಟಿಯನ್ನು ರಿಫ್ರೆಶ್ ಮಾಡಲಾಗುತ್ತಿದೆ…", liveNote:"ಲೈವ್ PMS ಡೇಟಾ · Overwatch ಸಂಭಾಷಣೆಯನ್ನು ತೆರೆದಿಡುವಾಗ ಬದಲಾವಣೆಗಳು ಇಲ್ಲಿ ಕಾಣುತ್ತವೆ.", dueIn:"ಆಗಮನ / ಡ್ಯೂ-ಇನ್", dueOut:"ನಿರ್ಗಮನ / ಡ್ಯೂ-ಔಟ್", inHouseLabel:"ತಂಗಿರುವ ಅತಿಥಿಗಳು"
   },
   "te-IN": {
    arrivals:"లైవ్ PMS నుండి నేటి రాకలను చూపిస్తున్నాను. రిజర్వేషన్‌ను తెరవడానికి “open [guest name]” లేదా నియంత్రిత చెక్-ఇన్ ప్రారంభించడానికి “prepare check-in [guest name]” అని చెప్పండి.",
    departures:"లైవ్ PMS నుండి నేటి బయలుదేరేవారిని చూపిస్తున్నాను.", inHouse:"లైవ్ PMS నుండి ప్రస్తుతం ఉంటున్న అతిథులను చూపిస్తున్నాను.",
    reservations:"రిజర్వేషన్లను తెరుస్తున్నాను. లైవ్ జాబితాను కుదించడానికి అతిథి పేరు చెప్పండి.", housekeeping:"లైవ్ హౌస్‌కీపింగ్ కార్యక్షేత్రాన్ని తెరుస్తున్నాను.", rates:"రేట్లు మరియు ఇన్వెంటరీని తెరుస్తున్నాను.", folios:"ఫోలియోలు మరియు ఇన్‌వాయిస్‌లను తెరుస్తున్నాను.",
    opened:"{guest} యొక్క లైవ్ రిజర్వేషన్‌ను తెరుస్తున్నాను.", preparing:"{guest} యొక్క లైవ్ చెక్-ఇన్ సిద్ధతను తెరుస్తున్నాను. ఏ మార్పుకైనా ముందు గది, ఫోలియో మరియు సిద్ధత అడ్డంకులను చూపిస్తాను.",
    noMatch:"లైవ్ రాకలు, బయలుదేరేవారు లేదా ఇన్-హౌస్ జాబితాలో “{name}” కు ఒక స్పష్టమైన సరిపోలిక దొరకలేదు. ముందు “show today’s arrivals” అని చెప్పి, తరువాత కనిపించే పేరునే సరిగ్గా చెప్పండి.",
    selected:"బాగుంది. నేను {language}లో మాట్లాడతాను. మీరు ఏమి చేయాలనుకుంటున్నారు?", live:"లైవ్ {focus}", refresh:"ప్రాపర్టీ-లోకల్ PMS జాబితాను రిఫ్రెష్ చేస్తున్నాను…", liveNote:"లైవ్ PMS డేటా · Overwatch సంభాషణను తెరిచి ఉంచినప్పుడు మార్పులు ఇక్కడ కనిపిస్తాయి.", dueIn:"రాకలు / డ్యూ-ఇన్‌లు", dueOut:"బయలుదేరేవారు / డ్యూ-అవుట్‌లు", inHouseLabel:"ఇన్-హౌస్ బసలు"
   }
  };
  const template = (copy[language] || copy["en-IN"])[key] || copy["en-IN"][key] || "";
  return template.replace(/\{(\w+)\}/gu, (_, name) => String(values[name] ?? ""));
 }
 function overwatchFocusLabel(focus) { return overwatchText(focus === "due_in" ? "dueIn" : focus === "due_out" ? "dueOut" : "inHouseLabel"); }
 function saveJarvisVoicePreference() { jarvisVoicePreference = { language:jarvisLanguage.value, style:jarvisVoiceStyle.value }; updateJarvisVoiceStatus(); }
 jarvisLanguage.addEventListener("change", saveJarvisVoicePreference); jarvisVoiceStyle.addEventListener("change", saveJarvisVoicePreference);
 function addJarvisMessage(text, className = "") { const item = node("p", className, text); jarvisMessages.append(item); jarvisMessages.scrollTop = jarvisMessages.scrollHeight; return item; }
 function rememberJarvis(role, text) { jarvisHistory.push({ role, text }); jarvisHistory = jarvisHistory.slice(-8); }
 function selectedJarvisVoice() { const voices = window.speechSynthesis?.getVoices?.() || []; const language = jarvisVoicePreference.language.toLowerCase(); const indian = voices.filter(voice => voice.lang.toLowerCase() === language || voice.lang.toLowerCase().startsWith(language.slice(0,2))); if (jarvisVoicePreference.style === "india-young-male") return indian.find(voice => /male|ravi|aarav|prabhat|hemant|india/i.test(`${voice.name} ${voice.voiceURI}`)) || indian[0]; return indian.find(voice => jarvisVoicePreference.style === "warm-feminine" ? /female|woman|zira|priya|kavya/i.test(voice.name) : true) || indian[0]; }
 function updateJarvisVoiceStatus() { if (!("speechSynthesis" in window)) { jarvisVoiceStatus.textContent = "Speech output is unavailable in this browser."; return; } const selected = selectedJarvisVoice(); jarvisVoiceStatus.textContent = selected ? `Using this browser’s closest available Indian voice: ${selected.name}.` : "No matching Indian voice is installed; text and microphone input still work."; }
 function setJarvisActivity(steps, active = steps.length - 1) { jarvisActivity.replaceChildren(...steps.map((step,index) => { const item = node("li", index === active ? "is-active" : "", step); return item; })); }
 function speakJarvis(text) { if (!("speechSynthesis" in window)) return false; window.dispatchEvent(new Event("overwatchspeechstart")); window.speechSynthesis.cancel(); const utterance = new SpeechSynthesisUtterance(text); utterance.lang = jarvisVoicePreference.language; utterance.rate = .96; const preferred = selectedJarvisVoice(); if (preferred) utterance.voice = preferred; utterance.onend = () => window.dispatchEvent(new Event("overwatchspeechend")); utterance.onerror = () => window.dispatchEvent(new Event("overwatchspeechend")); window.speechSynthesis.speak(utterance); return true; }
 function addJarvisReply(text) { addJarvisMessage(text); const controls = node("div", "jarvis-reply-controls"); const speak = node("button", "quiet compact", "Speak reply"); speak.type = "button"; speak.addEventListener("click", () => { if (!speakJarvis(text)) addJarvisMessage("Speech output is unavailable in this browser.", "jarvis-warning"); }); controls.append(speak); jarvisMessages.append(controls); speakJarvis(text); }
 function focusTodayLane(focus) { if (!focus) return; todayView.dataset.jarvisFocus = focus; todayBoundary.textContent = `Showing ${jarvisFocusLabels[focus]} requested through Yellow. Use Today in the navigation to return to the full operational view.`; const lane = todayLanes.find((item) => item.dataset.todayLane === focus); lane?.scrollIntoView({ behavior:"smooth", block:"start" }); }
 function addJarvisPip(reply) { if (!reply.navigation) return; const card = node("article", "jarvis-pip"); const title = reply.focus ? jarvisFocusLabels[reply.focus] : String(reply.navigation); card.append(node("h3", "", title)); const rows = reply.focus && todayLaneState[reply.focus] ? todayLaneState[reply.focus].rows.slice(0, 4) : []; if (rows.length) { const list = el("ul"); for (const row of rows) list.append(node("li", "", `${row.primaryGuestDisplayName} · ${row.sellableUnitLabel || row.unitTypeLabel}`)); card.append(list); } else card.append(node("p", "", "Refreshing the current operational records…")); jarvisPips.querySelector(".jarvis-pips-empty")?.remove(); jarvisPips.prepend(card); }
 function addJarvisLiveSnapshot(reply) {
  if (!reply.focus || !todayLaneState[reply.focus]) return;
  const snapshot = node("section", "jarvis-live-snapshot");
  snapshot.append(node("h3", "", overwatchText("live", { focus:overwatchFocusLabel(reply.focus) })));
  const rows = todayLaneState[reply.focus].rows.slice(0, 6);
  if (rows.length === 0) snapshot.append(node("p", "", overwatchText("refresh")));
  else {
   const list = el("ul");
   for (const row of rows) {
    const item = node("li", "");
    item.append(node("strong", "", row.primaryGuestDisplayName || "Guest"), node("span", "", ` · ${row.confirmationNo || "Reservation"} · ${row.sellableUnitLabel || row.unitTypeLabel || "Room pending"}`));
    list.append(item);
   }
   snapshot.append(list);
  }
  snapshot.append(node("p", "jarvis-live-note", overwatchText("liveNote")));
  jarvisMessages.append(snapshot); jarvisMessages.scrollTop = jarvisMessages.scrollHeight;
 }
 function addJarvisCheckInProgress() {
  const reservation = reservationDetailData?.reservation;
  if (!reservation || reservation.status !== "due_in") return;
  const card = node("section", "jarvis-live-snapshot jarvis-checkin-progress");
  card.append(node("h3", "", `Check-in progress · ${reservation.primaryGuestDisplayName || reservation.confirmationNo}`));
  if (!checkInReadinessData) card.append(node("p", "", "Refreshing room, folio and arrival evidence from the PMS…"));
  else if (checkInReadinessData.canCheckIn) {
   card.append(node("p", "", "Ready for the named confirmation. No check-in has been performed."));
  } else {
   card.append(node("p", "", "Paused safely — these live PMS items must be resolved before check-in:"));
   const list = el("ul");
   for (const blocker of checkInReadinessData.blockers || []) list.append(node("li", "", checkInBlockerLabels[blocker] || String(blocker).replaceAll("_", " ")));
   card.append(list);
  }
  card.append(node("p", "jarvis-live-note", "Live server check · Overwatch will never bypass room, folio or confirmation controls."));
  jarvisMessages.append(card); jarvisMessages.scrollTop = jarvisMessages.scrollHeight;
 }
 function liveReservationMatch(name) {
  const query = String(name || "").trim().toLocaleLowerCase();
  if (query.length < 2) return null;
  const matches = Object.values(todayLaneState).flatMap((lane) => lane.rows || []).filter((row) =>
   String(row.primaryGuestDisplayName || "").toLocaleLowerCase().includes(query));
  return matches.length === 1 ? matches[0] : null;
 }
 function localOverwatchReply(message) {
  const value = message.toLocaleLowerCase();
  const reply = (answer, navigation, focus = null) => ({ answer, navigation, focus, requiresConfirmation:false, local:true });
  // Keep the no-token demo hand-off usable after a regional language is chosen.
  // These are deliberately narrow “verb + guest name” forms: a matching live row is
  // still required before a reservation can be opened or a check-in prepared.
  const reservationCommand = /(?:(?:open|expand|pop[ -]?up)(?:\s+(?:reservation|booking|guest))?|show\s+(?:reservation|booking|guest)|prepare\s+(?:a\s+)?check[ -]?in\s+(?:for\s+)?|खोलें\s+(?:आरक्षण\s+)?|उघडा\s+(?:आरक्षण\s+)?|ತೆರೆಯಿರಿ\s+(?:ಕಾಯ್ದಿರಿಸುವಿಕೆ\s+)?|తెరవండి\s+(?:రిజర్వేషన్‌ను?\s+)?|चेक[ -]?इन\s+तैयार\s+करें\s+(?:के\s+लिए\s+)?|चेक[ -]?इन\s+तयार\s+करा\s+(?:साठी\s+)?|ಚೆಕ್[ -]?ಇನ್\s+ಸಿದ್ಧಪಡಿಸಿ\s+|చెక్[ -]?ఇన్\s+సిద్ధం\s+చేయండి\s+)(.+)/iu.exec(message);
  if (reservationCommand) {
   const requestedName = reservationCommand[1].replace(/[?.!]+$/u, "").trim();
   const row = liveReservationMatch(requestedName);
   if (row) {
    const prepare = /prepare\s+(?:a\s+)?check[ -]?in|चेक[ -]?इन\s+तैयार\s+करें|चेक[ -]?इन\s+तयार\s+करा|ಚೆಕ್[ -]?ಇನ್\s+ಸಿದ್ಧಪಡಿಸಿ|చెక్[ -]?ఇన్\s+సిద్ధం\s+చేయండి/iu.test(message);
    return { answer: overwatchText(prepare ? "preparing" : "opened", { guest:row.primaryGuestDisplayName }), navigation:"reservations", reservationId:row.reservationId, workbench:prepare ? "check-in" : null, requiresConfirmation:false, local:true };
   }
   return reply(overwatchText("noMatch", { name:requestedName }), "today", "due_in");
  }
  if (/(arrival|arrivals|due[ -]?in|आजचे आगमन|आगमन|ఆగమనం|రాకలు|ಆಗಮನಗಳು)/u.test(value)) return reply(overwatchText("arrivals"), "today", "due_in");
  if (/(departure|departures|due[ -]?out|निर्गमन|బయలుదేరు|ನಿರ್ಗಮನ)/u.test(value)) return reply(overwatchText("departures"), "today", "due_out");
  if (/(in[ -]?house|staying|checked[ -]?in|मुक्काम|నివాస|ತಂಗಿರುವ)/u.test(value)) return reply(overwatchText("inHouse"), "today", "in_house");
  if (/(show|open).*(reservation|booking)|reservation.*(show|open)|booking.*(show|open)/u.test(value)) return reply(overwatchText("reservations"), "reservations");
  if (/(show|open).*(room|housekeeping|cleaning|maintenance)|housekeeping/u.test(value)) return reply(overwatchText("housekeeping"), "housekeeping");
  if (/(show|open).*(rate|pricing|inventory)|rate.*(show|open)|pricing/u.test(value)) return reply(overwatchText("rates"), "rates");
  if (/(show|open).*(folio|invoice)|folio|invoice/u.test(value)) return reply(overwatchText("folios"), "folios");
  return null;
 }
 function presentJarvisReply(reply) {
  const answer = String(reply.answer || "Overwatch could not answer that request."); addJarvisReply(answer); rememberJarvis("assistant", answer); addJarvisPip(reply);
  setJarvisActivity(["Request understood", reply.navigation ? "Matching the operational view" : "Preparing the response", reply.requiresConfirmation ? "Change requires your visible confirmation" : "Answer ready"], 2);
  if (reply.requiresConfirmation) addJarvisMessage("This is an operational change. Overwatch can prepare it, but you must review and confirm it in the relevant workflow.", "jarvis-warning");
  const view = jarvisViews[reply.navigation];
  if (reply.reservationId) {
   setView("reservations", false);
   void openReservationDetail(reply.reservationId, { workbench:reply.workbench || null }).then(() => {
    if (reply.workbench !== "check-in") return;
    window.setTimeout(() => addJarvisCheckInProgress(), 650);
   });
   return;
  }
  if (view) {
   setView(view);
   if (view === "today") { focusTodayLane(reply.focus); addJarvisLiveSnapshot(reply); }
  }
 }
 function addOverwatchSuggestedPrompts() {
  jarvisMessages.querySelector(".overwatch-suggested-prompts")?.remove();
  const labels = ({
   "en-IN":["Show today’s arrivals", "Show departures", "Open Aarav’s profile", "Prepare Aarav check-in"],
   "hi-IN":["आज के आगमन दिखाएँ", "प्रस्थान दिखाएँ", "आरव की प्रोफ़ाइल खोलें", "आरव का चेक-इन तैयार करें"],
   "mr-IN":["आजचे आगमन दाखवा", "निर्गमन दाखवा", "आरवचे प्रोफाइल उघडा", "आरवचे चेक-इन तयार करा"],
   "kn-IN":["ಇಂದಿನ ಆಗಮನಗಳನ್ನು ತೋರಿಸಿ", "ನಿರ್ಗಮನಗಳನ್ನು ತೋರಿಸಿ", "ಆರವ್ ಅವರ ಪ್ರೊಫೈಲ್ ತೆರೆಯಿರಿ", "ಆರವ್ ಚೆಕ್-ಇನ್ ಸಿದ್ಧಪಡಿಸಿ"],
   "te-IN":["నేటి రాకలను చూపండి", "బయలుదేరేవారిని చూపండి", "ఆరవ్ ప్రొఫైల్ తెరవండి", "ఆరవ్ చెక్-ఇన్ సిద్ధం చేయండి"],
  }[jarvisVoicePreference.language] || ["Show today’s arrivals", "Show departures", "Open Aarav’s profile", "Prepare Aarav check-in"]);
  const prompts = [[labels[0], "show today’s arrivals"], [labels[1], "show today’s departures"], [labels[2], "open Aarav Mehta"], [labels[3], "prepare check-in Aarav Mehta"]];
  const choices = node("div", "overwatch-suggested-prompts");
  for (const [label, message] of prompts) {
   const choice = node("button", "overwatch-suggested-prompt", label); choice.type = "button";
   choice.addEventListener("click", () => void submitJarvisMessage(message)); choices.append(choice);
  }
  jarvisMessages.append(choices);
 }
 function offerOverwatchLanguage() { if (overwatchLanguagePrompted) return; overwatchLanguagePrompted = true; addJarvisMessage("Hello, I’m Overwatch. Which language would you like to speak?", "overwatch-greeting"); const choices = node("div", "overwatch-language-choices"); for (const option of [...jarvisLanguage.options]) { const choice = node("button", "overwatch-language-choice", option.text); choice.type = "button"; choice.addEventListener("click", () => { jarvisLanguage.value = option.value; saveJarvisVoicePreference(); overwatchLanguageSelected = true; choices.remove(); const reply = overwatchText("selected", { language:option.text }); addJarvisMessage(reply, "overwatch-greeting"); addOverwatchSuggestedPrompts(); speakJarvis(reply); jarvisMessage.focus({ preventScroll:true }); }); choices.append(choice); } jarvisMessages.append(choices); }
 function openJarvis() { if (!jarvisDialog.open) jarvisDialog.showModal(); jarvisDialog.scrollTop = 0; updateJarvisVoiceStatus(); offerOverwatchLanguage(); jarvisMessage.focus({ preventScroll:true }); }
 jarvisLaunch.addEventListener("click", openJarvis);
 jarvisDialog.addEventListener("close", () => { overwatchLanguagePrompted = false; overwatchLanguageSelected = false; });
 jarvisClear.addEventListener("click", () => { jarvisHistory = []; jarvisMessages.replaceChildren(node("p", "", "This device’s session transcript is clear. What would you like Overwatch to do next?")); overwatchLanguagePrompted = false; overwatchLanguageSelected = false; offerOverwatchLanguage(); setJarvisActivity(["Waiting for your request", "Nothing from the earlier session is retained"], 0); });
 $("#nav-today").addEventListener("click", () => { delete todayView.dataset.jarvisFocus; todayBoundary.textContent = "Due-in, due-out and in-house reservations overlapping this property-local day. Each list is one bounded server page, not a hotel-wide total."; });
 async function submitJarvisMessage(message) {
  if (!message) return; addJarvisMessage(message, "jarvis-user"); rememberJarvis("user", message); jarvisMessage.value = ""; setJarvisActivity(["Request received", "Understanding your instruction", "Checking the permitted hotel context"], 1);
  const localReply = localOverwatchReply(message);
  if (localReply) { presentJarvisReply(localReply); return; }
  try {
   const reply = await request("/api/v1/jarvis:ask", { method:"POST", body:JSON.stringify({ message, language:jarvisVoicePreference.language, history:jarvisHistory.slice(-3, -1) }) });
   presentJarvisReply(reply);
  } catch (error) { setJarvisActivity(["Request received", "Overwatch could not complete this step"], 1); addJarvisMessage(error instanceof Error ? error.message : "Overwatch could not complete that request.", "jarvis-warning"); }
 }
 jarvisForm.addEventListener("submit", (event) => { event.preventDefault(); void submitJarvisMessage(jarvisMessage.value.trim()); });
 function wakePhrase(value) { return /(?:^|\s)(?:overwatch|ओवरवॉच|ಓವರ್[\s-]?ವಾಚ್|ఓవర్[\s-]?వాచ్)(?:[\s,.:]|$)/iu.test(value); }
 function afterWakePhrase(value) { return value.replace(/^.*?(?:overwatch|ओवरवॉच|ಓವರ್[\s-]?ವಾಚ್|ఓవర్[\s-]?వాచ్)[\s,.:]*/iu, "").trim(); }
 const Recognition = window.SpeechRecognition || window.webkitSpeechRecognition;
 if (Recognition) {
  const recognition = new Recognition();
 // SpeechRecognition may mark a short human hesitation as a final fragment. Keep
 // collecting for a full natural sentence rather than turning the microphone over
 // after a one-word pause.
 const overwatchNaturalPauseMs = 3000;
 let voiceTurnTimer = null;
 let voiceTurnSegments = [];
 let recognitionStoppingForReply = false;
  let overwatchConversationUntil = 0;
  recognition.lang = jarvisVoicePreference.language;
  recognition.interimResults = true;
  recognition.continuous = true;
  const flushVoiceTurn = () => {
   const transcript = voiceTurnSegments.join(" ").replace(/\s+/g, " ").trim();
   voiceTurnSegments = [];
   if (!transcript) return;
   openJarvis();
   const isWakeCommand = wakePhrase(transcript);
   const command = isWakeCommand ? afterWakePhrase(transcript) : transcript;
   if (isWakeCommand) overwatchConversationUntil = Date.now() + 90000;
   const localFollowUp = Date.now() < overwatchConversationUntil && localOverwatchReply(command);
   if (!isWakeCommand && !localFollowUp) { jarvisListening.textContent = "Listening for “Overwatch”"; return; }
   if (command) void submitJarvisMessage(command);
   else {
    const prompt = "Overwatch is ready. What would you like to see?";
    addJarvisMessage(prompt, "jarvis-warning");
    speakJarvis(prompt);
   }
  };
  recognition.addEventListener("result", event => {
   ...
   if (!transcript) return;
   openJarvis();
   jarvisListening.textContent = `Hearing: ${transcript}`;
   if (!result.isFinal) return;
   if (!overwatchLanguageSelected) offerOverwatchLanguage();
   voiceTurnSegments.push(transcript);
   if (voiceTurnTimer) window.clearTimeout(voiceTurnTimer);
   // Leave room for a natural phrase break. A final recognition fragment alone
   // never submits a command; Overwatch waits long enough for a human sentence
   // to continue before it turns the mic over to the response.
   voiceTurnTimer = window.setTimeout(() => {
    voiceTurnTimer = null;
    flushVoiceTurn();
   }, overwatchNaturalPauseMs);
  });
  recognition.addEventListener("end", () => {
   jarvisMic.setAttribute("aria-pressed", "false");
   if (jarvisVoiceEnabled && !recognitionStoppingForReply) {
    try { recognition.start(); jarvisMic.setAttribute("aria-pressed", "true"); } catch {}
   }
  });
  recognition.addEventListener("error", event => {
   if (event.error === "aborted" && recognitionStoppingForReply) return;
   jarvisVoiceEnabled = false;
   jarvisMic.setAttribute("aria-pressed", "false");
   jarvisListening.textContent = "Voice input could not start";
   addJarvisMessage("Overwatch could not start voice input. Check the browser microphone permission, then try again.", "jarvis-warning");
  });
  const resumeVoiceRecognition = () => {
   recognitionStoppingForReply = false;
   if (jarvisVoiceEnabled) { try { recognition.start(); jarvisMic.setAttribute("aria-pressed", "true"); jarvisListening.textContent = "Listening for Overwatch"; } catch {} }
  };
  window.addEventListener("overwatchspeechstart", () => {
   recognitionStoppingForReply = true;
   if (voiceTurnTimer) { window.clearTimeout(voiceTurnTimer); voiceTurnTimer = null; }
   if (jarvisVoiceEnabled) { try { recognition.abort(); } catch {} }
  });
  window.addEventListener("overwatchspeechend", resumeVoiceRecognition);
  jarvisMic.addEventListener("click", async () => {
   openJarvis();
   if (jarvisVoiceEnabled) {
    ...
    recognition.abort();
    jarvisMic.setAttribute("aria-pressed", "false");
    jarvisListening.textContent = "Voice listening paused";
    setJarvisActivity(["Voice session paused", "Tap the microphone to continue"], 0);
    return;
   }
   try {
    if (!navigator.mediaDevices?.getUserMedia) throw new Error("Microphone permission is unavailable in this browser.");
    const stream = await navigator.mediaDevices.getUserMedia({ audio:true });
    stream.getTracks().forEach(track => track.stop());
    recognition.lang = jarvisVoicePreference.language;
    recognitionStoppingForReply = false;
    jarvisVoiceEnabled = true;
    jarvisListening.textContent = "Listening for Overwatch";
    setJarvisActivity(["Microphone enabled", "Listening continuously — pause naturally when you are done"], 1);
    jarvisMic.setAttribute("aria-pressed", "true");
    recognition.start();
   } catch (error) {
    jarvisVoiceEnabled = false;
    jarvisMic.setAttribute("aria-pressed", "false");
    jarvisListening.textContent = "Microphone permission required";
    addJarvisMessage(error instanceof Error ? `${error.message} Use the browser’s site controls to allow the microphone, then tap the mic again.` : "Allow microphone access in the browser, then tap the mic again.", "jarvis-warning");
   }
  });
