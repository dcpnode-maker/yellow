...
export class ReservationOfferSearchService {
  ...
  async search(tx: Tx, value: ReservationOfferSearchInput): Promise<ReservationOfferSearchResult> {
    ...
    const availableInventory = (await this.#availability.search(tx, {
    ...
    const unpublishedPlanIds = new Set<string>();
    for (const inventoryOption of inventory) {
      for (const plan of plans) {
        // A missing active release is plan-wide. Once it has been established for
        // one sellable, avoid repeating the same release lookup for every room.
        if (unpublishedPlanIds.has(plan.id)) continue;
        try {
          ...
          if (reason === "publication_unavailable" && error.message === "Active rate release was not found") {
            unpublishedPlanIds.add(plan.id);
          }
