...
export const MAX_FOLIO_HOSTED_DEPOSITS = 50;
export const MAX_FOLIO_DEPOSIT_INSTRUMENTS = 50;
...
export interface FolioHostedDepositWorkbenchInput {
  readonly tenantId: string;
...
export interface HostedDepositInstrument {
  readonly instrumentId: string;
  readonly kind: "card_network_token" | "upi_vpa";
  readonly brand: string | null;
  readonly last4: string | null;
  readonly expiry: string | null;
  readonly psp: string | null;
}

export interface FolioHostedDepositWorkbench {
  readonly propertyNode: string;
  readonly folioId: string;
  readonly deposits: readonly HostedDepositStatus[];
  readonly instruments: readonly HostedDepositInstrument[];
}
...
export class HostedDepositService {
  ...
  async workbenchForFolio(input: FolioHostedDepositWorkbenchInput): Promise<FolioHostedDepositWorkbench> {
    const tenantId = uuid(input.tenantId, "tenantId");
    const propertyNode = uuid(input.propertyNode, "propertyNode");
    const folioId = uuid(input.folioId, "folioId");
    return this.#database.withTenantTransaction(tenantId, async (tx) => {
      const owner = (await tx<Array<{
        party_id: string;
        currency: string;
        folio_open: boolean;
        account_open: boolean;
      }>>`
        SELECT account.party_id,account.currency::text,
               folio.status='open' folio_open,account.status='open' account_open
          FROM folio
          JOIN account ON account.tenant_id=folio.tenant_id AND account.id=folio.account_id
          JOIN party ON party.tenant_id=account.tenant_id AND party.id=account.party_id
          JOIN org_node property ON property.tenant_id=account.tenant_id
            AND property.id=account.property_node AND property.kind='property'
         WHERE folio.tenant_id=${tenantId}::uuid AND folio.id=${folioId}::uuid
           AND account.property_node=${propertyNode}::uuid AND account.role='guest'
           AND property.currency=account.currency
      `)[0];
      if (!owner) throw new HostedDepositNotFoundError("Hosted deposit folio was not found");

      const requests = await tx<RequestRow[]>`
        SELECT request.id,request.tenant_id,request.property_node,property.name property_name,
               request.folio_id,folio.folio_no folio_reference,request.operation_id,
               request.amount_minor::text,request.currency::text,request.generation,
               request.expires_at,request.revoked_at,operation.purpose,
               operation.deposit_account_id,operation.guest_account_id
          FROM hosted_payment_request request
          JOIN payment_operation operation ON operation.tenant_id=request.tenant_id
            AND operation.id=request.operation_id
          JOIN org_node property ON property.tenant_id=request.tenant_id
            AND property.id=request.property_node
          JOIN folio ON folio.tenant_id=request.tenant_id AND folio.id=request.folio_id
         WHERE request.tenant_id=${tenantId}::uuid
           AND request.property_node=${propertyNode}::uuid
           AND request.folio_id=${folioId}::uuid
           AND request.operation_purpose='deposit' AND operation.purpose='deposit'
         ORDER BY request.generation DESC,request.id DESC
         LIMIT ${MAX_FOLIO_HOSTED_DEPOSITS}
      `;
      const deposits: HostedDepositStatus[] = [];
      for (const request of requests) deposits.push(await materializeStatus(tx, request));
      const instrumentRows = owner.folio_open && owner.account_open ? await tx<InstrumentRow[]>`
        SELECT instrument.id,instrument.kind,instrument.brand,instrument.last4::text,
               instrument.expiry,instrument.psp
          FROM payment_instrument instrument
          JOIN tx_code code ON code.code=CASE instrument.kind
            WHEN 'card_network_token' THEN 'CARD_PAYMENT' ELSE 'UPI_PAYMENT' END
          JOIN tx_code_route route ON route.tenant_id=instrument.tenant_id
            AND route.property_node=${propertyNode}::uuid AND route.currency=${owner.currency}::char(3)
            AND route.tx_code=code.code
          JOIN account clearing ON clearing.tenant_id=route.tenant_id
            AND clearing.id=route.debit_account_id
          JOIN tx_code_route deposit_route ON deposit_route.tenant_id=instrument.tenant_id
            AND deposit_route.property_node=${propertyNode}::uuid
            AND deposit_route.currency=${owner.currency}::char(3) AND deposit_route.tx_code='DEP'
          JOIN account deposit ON deposit.tenant_id=deposit_route.tenant_id
            AND deposit.property_node=deposit_route.property_node
            AND deposit.currency=deposit_route.currency AND deposit.id=deposit_route.credit_account_id
            AND deposit.role='deposit_liability' AND deposit.status='open'
         WHERE instrument.tenant_id=${tenantId}::uuid AND instrument.party_id=${owner.party_id}::uuid
           AND instrument.status='active' AND instrument.kind IN ('card_network_token','upi_vpa')
           AND instrument.token IS NOT NULL AND instrument.token ~ '^[!-~]{8,200}$'
           AND instrument.token !~ '^[0-9]{12,19}$'
           AND instrument.psp ~ '^[a-z][a-z0-9._-]{0,63}$'
           AND code.grp='payment' AND code.default_dr=CASE instrument.kind
             WHEN 'card_network_token' THEN 'card_clearing' ELSE 'upi_clearing' END
           AND code.default_cr='guest' AND clearing.status='open'
           AND clearing.role=CASE instrument.kind
             WHEN 'card_network_token' THEN 'card_clearing' ELSE 'upi_clearing' END
           AND clearing.property_node=${propertyNode}::uuid AND clearing.currency=${owner.currency}::char(3)
         ORDER BY instrument.kind,instrument.brand ASC NULLS LAST,instrument.last4 ASC NULLS LAST,
           instrument.expiry ASC NULLS LAST,instrument.psp ASC NULLS LAST,instrument.id
         LIMIT ${MAX_FOLIO_DEPOSIT_INSTRUMENTS}
      ` : [];
      const instruments = instrumentRows.map((instrument) => Object.freeze({
