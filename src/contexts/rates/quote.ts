...
export class RateQuoteService {
  ...
  async resolve(tx: Tx, value: ResolveRateQuoteInput): Promise<RateQuote> {
    ...
    let release: RatePlanRelease;
    try {
      release = await this.#publication.getActiveRelease(tx, input.propertyNode, input.ratePlanId);
    } catch (error) {
      // Offer search evaluates every active plan. A plan which has not yet passed
      // its own publication workflow is an unavailable offer, never a wholesale
      // availability outage for the plans that are already live.
      if (error instanceof RatePublicationNotFoundError) {
        throw new RateQuoteNotFoundError("Active rate release was not found");
      }
      throw error;
    }
