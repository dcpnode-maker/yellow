...
export class LocalLoginService {
  ...
  async authenticate(input: LocalLoginInput, sourceKey = "unknown"): Promise<LocalLoginResult | null> {
    ...
    return this.#authenticateNormalized(normalized, accountKey);
  }

  /** Only the read-only, environment-gated synthetic public demo may call this. */
  async authenticatePublicReadOnlyDemo(input: LocalLoginInput, sourceKey = "unknown"): Promise<LocalLoginResult | null> {
    const normalized = normalize(input);
    if (!normalized) return null;
    const gate = this.#guard.consumeSource(sourceKey);
    if (!gate.allowed) throw new LocalLoginLimitedError(gate.retryAfterSeconds);
    return this.#authenticateNormalized(normalized);
  }

  async #authenticateNormalized(normalized: NormalizedLogin, accountKey?: string): Promise<LocalLoginResult | null> {
    ...
    if (!user || !verification.value) {
      if (accountKey) this.#guard.recordFailure(accountKey);
      return null;
    }
    ...
    if (accountKey) this.#guard.recordSuccess(accountKey);
    return {
      accessToken,
      tokenType: "Bearer",
