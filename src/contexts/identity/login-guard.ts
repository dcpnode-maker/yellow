...
export class LocalLoginGuard {
  ...
  /**
   * The public synthetic-demo route has no user-supplied credentials and is
   * additionally made read-only at the HTTP boundary. It still needs a bounded
   * per-visitor admission gate, but must not share the real staff account bucket
   * between every visitor behind a reverse proxy.
   */
  consumeSource(sourceKey: string): LocalLoginGuardDecision {
    if (sourceKey.length === 0 || sourceKey.length > 128) {
      return Object.freeze({ allowed: false, retryAfterSeconds: MAX_RETRY_AFTER_SECONDS });
    }
    const now = this.#monotonicNow();
    const source = this.#source(sourceKey, now);
    if (!source) return Object.freeze({ allowed: false, retryAfterSeconds: this.#admissionRetry(this.#sources, now, SOURCE_REFILL_PERIOD_MS) });
    const cost = SOURCE_REFILL_PERIOD_MS / SOURCE_REFILL_TOKENS;
    refill(source, now, SOURCE_CAPACITY * cost);
    const retryMs = source.creditMs >= cost ? 0 : cost - source.creditMs;
    if (retryMs === 0) source.creditMs -= cost;
    touch(this.#sources, sourceKey, source, now);
    return retryMs > 0
      ? Object.freeze({ allowed: false, retryAfterSeconds: boundedRetryAfter(retryMs) })
      : Object.freeze({ allowed: true });
