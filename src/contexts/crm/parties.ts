...
const CONTACT_KINDS: readonly PartyContactKind[] = [
  "email",
  "phone",
  "whatsapp",
];
...
export interface DuplicateCandidate extends Readonly<
...
export interface UpdatePartyProfileResult {
  readonly party: PartyProfile;
...
interface StoredUpdateBody extends Readonly<Record<string, JsonValue>> {
  readonly partyId: string;
  readonly kind: PartyKind;
  readonly legalName: string | null;
  readonly roles: readonly PartyRole[];
  readonly contacts: readonly PartyContactHint[];
  readonly changed: boolean;
  readonly changedFields: readonly ("display_name" | "legal_name")[];
}
...
export class PartyDuplicateReviewRequiredError extends Error {
  ...
  constructor(candidates: readonly DuplicateCandidate[]) {
    super(
      "Current possible duplicates must be reviewed and acknowledged exactly",
    );
...
function normalizedName(name: string, value: unknown, maximum: number): string {
  if (typeof value !== "string")
    throw new PartyProfileValidationError(`${name} must be a string`);
  ...
  if (
    ...
    throw new PartyProfileValidationError(
      `${name} must contain 1-${maximum} visible characters`,
    );
  }
  return normalized;
}
...
function normalizedContact(
  ...
  if (typeof input.value !== "string") {
    throw new PartyProfileValidationError(
      `contacts[${index}].value must be a string`,
    );
  }
  const value =
    input.kind === "email"
  ...
  if (input.kind === "email") {
    if (value.length > 254 || !EMAIL.test(value)) {
      throw new PartyProfileValidationError(
        `contacts[${index}].value must be a valid email address`,
      );
    }
  } else if (!E164.test(value)) {
    throw new PartyProfileValidationError(
      `contacts[${index}].value must be canonical E.164`,
    );
  }
  if (input.isPrimary !== undefined && typeof input.isPrimary !== "boolean") {
    throw new PartyProfileValidationError(
      `contacts[${index}].isPrimary must be boolean`,
    );
  }
...
function normalizeCreate(input: CreatePartyProfileInput): NormalizedCreate {
  if (typeof input !== "object" || input === null || Array.isArray(input)) {
    throw new PartyProfileValidationError(
      "Party create input must be an object",
    );
  }
  ...
  if (input.envelope.operation !== "party.created") {
    throw new PartyProfileValidationError(
      "audit operation must be party.created",
    );
  }
  ...
  if (
    ...
    input.roles.length > PARTY_ROLES.length
  ) {
  ...
  for (const role of input.roles) {
    if (!PARTY_ROLES.includes(role))
      throw new PartyProfileValidationError("roles contain an invalid value");
    if (roleSet.has(role))
      throw new PartyProfileValidationError("roles must be unique");
    roleSet.add(role);
  }
  if (!Array.isArray(input.contacts) || input.contacts.length > 6) {
    throw new PartyProfileValidationError(
      "contacts must contain at most 6 entries",
    );
  }
  ...
  for (const contact of contacts) {
    ...
    if (contactKeys.has(key))
      throw new PartyProfileValidationError(
        "contacts must be unique by kind and value",
      );
    contactKeys.add(key);
    if (contact.isPrimary) {
      if (primaryKinds.has(contact.kind)) {
        throw new PartyProfileValidationError(
          "only one primary contact is allowed per kind",
        );
      }
  ...
  if (
    !Array.isArray(input.acknowledgedDuplicatePartyIds) ||
    input.acknowledgedDuplicatePartyIds.length > MAX_DUPLICATES
  ) {
    throw new PartyProfileValidationError(
      "acknowledgedDuplicatePartyIds must contain at most 50 entries",
    );
  }
  ...
  if (new Set(acknowledged).size !== acknowledged.length) {
    throw new PartyProfileValidationError(
      "acknowledgedDuplicatePartyIds must be unique",
    );
  }
  const sortedAcknowledged = [...acknowledged].sort();
  if (!sameIds(acknowledged, sortedAcknowledged)) {
    throw new PartyProfileValidationError(
      "acknowledgedDuplicatePartyIds must be sorted",
    );
  }
  return Object.freeze({
    kind: input.kind,
    ...
    legalName:
      input.legalName === undefined || input.legalName === null
        ? null
...
function normalizeUpdate(input: UpdatePartyProfileInput): Readonly<{
  partyId: string;
  displayName: string;
  ...
  if (typeof input !== "object" || input === null || Array.isArray(input)) {
    throw new PartyProfileValidationError(
      "Party update input must be an object",
    );
  }
  requireExactKeys("Party update input", input, [
    "partyId",
    "displayName",
    "legalName",
    "idempotencyKey",
  ...
  if (input.envelope.operation !== "party.updated") {
    throw new PartyProfileValidationError(
      "audit operation must be party.updated",
    );
  }
  if (
    typeof input.idempotencyKey !== "string" ||
  ...
  return Object.freeze({
    partyId: requireUuid("partyId", input.partyId),
    displayName: normalizedName("displayName", input.displayName, 200),
    legalNameProvided: input.legalName !== undefined,
    legalName:
      input.legalName === undefined || input.legalName === null
        ? null
        : normalizedName("legalName", input.legalName, 300),
...
function asContactKind(value: string): PartyContactKind {
  if (CONTACT_KINDS.includes(value as PartyContactKind))
    return value as PartyContactKind;
  ...
  return Object.freeze(
    rows
      .map((row) => {
        ...
        return Object.freeze({
          kind,
          hint: maskContact(kind, row.value),
...
function profile(
  ...
  if (row.status !== "active")
    throw new Error("Stored Party status is not active");
  return Object.freeze({
    partyId: row.id,
    ...
    roles: Object.freeze(
      roles
        .filter(({ party_id }) => party_id === row.id)
        .map(({ role }) => asRole(role)),
    ),
...
export class PartyProfileService {
  ...
  async search(
    tx: Tx,
    input: SearchPartyProfilesInput,
    ...
    const emailQuery = query.includes("@")
      ? query.normalize("NFKC").trim().toLowerCase()
      : null;
    const e164Query = E164.test(query) ? query : null;
    const isNameQuery =
      uuidQuery === null && emailQuery === null && e164Query === null;
    const prefixQuery =
      isNameQuery && query.length === 2
    ...
    `;
    const related = await relatedRows(
      tx,
      tenantId,
      rows.map(({ id }) => id),
    );
    return Object.freeze(
      rows.map((row) => profile(row, related.roles, related.contacts)),
    );
  }
  ...
  async #duplicates(
    ...
    if (!displayNameKey)
      throw new Error("PostgreSQL did not return the Party display-name key");
    ...
    const emailValues = normalized.contacts
      .filter(({ kind }) => kind === "email")
      .map(({ value }) => value);
    const phoneValues = normalized.contacts
      .filter(({ kind }) => kind !== "email")
      .map(({ value }) => value);
    ...
    if (rows.length > MAX_DUPLICATES) {
      throw new PartyProfileValidationError(
        "duplicate review exceeds 50 candidates; add a distinguishing contact",
      );
    }
    const related = await relatedRows(
      tx,
    ...
    return Object.freeze(
      rows.map((row) => {
        const contacts = related.contacts.filter(
          ({ party_id }) => party_id === row.id,
        );
        const reasons = new Set<DuplicateReason>();
        if (row.display_name_match) reasons.add("display_name");
        for (const contact of contacts) {
          if (contact.kind === "email" && emailValues.includes(contact.value))
            reasons.add("email");
          if (
            (contact.kind === "phone" || contact.kind === "whatsapp") &&
            phoneValues.includes(contact.value)
          ) {
  ...
  ): Promise<CreatePartyProfileResult> {
    const normalized = normalizeCreate(input);
    const outcome = await this.#idempotency.execute<StoredCreateBody>(
      tx,
      {
        ...
        request: {
          ...
          acknowledgedDuplicatePartyIds:
            normalized.acknowledgedDuplicatePartyIds,
        },
      },
      async (commandTx) => {
      ...
      `;
        ...
        if (!party)
          throw new Error("PostgreSQL did not return the created Party");
        ...
        const contactKinds = Object.freeze(
          [...new Set(normalized.contacts.map(({ kind }) => kind))].sort(),
        );
  ...
  async update(
  ...
  ): Promise<UpdatePartyProfileResult> {
    const normalized = normalizeUpdate(input);
    const outcome = await this.#idempotency.execute<StoredUpdateBody>(
      tx,
      {
        tenantId: input.envelope.tenantId,
        operation: "profiles.party.update",
        key: normalized.idempotencyKey,
        request: {
          actorId: input.envelope.actorId,
          propertyNode: input.envelope.propertyNode,
          ...
          displayName: normalized.displayName,
          legalNameProvided: normalized.legalNameProvided,
          legalName: normalized.legalName,
        },
      },
      async (commandTx) => {
        const rows = await commandTx<PartyRow[]>`
        SELECT id, kind, display_name, legal_name, status
        FROM party
        WHERE id = ${normalized.partyId}::uuid
          AND tenant_id = ${input.envelope.tenantId}::uuid
          AND tenant_id = current_setting('app.tenant_id', true)::uuid
      `;
        const party = rows[0];
        if (!party || rows.length !== 1 || party.status !== "active") {
          throw new PartyProfileValidationError(
            "active Party was not found in this tenant",
          );
        }
        const legalName = normalized.legalNameProvided
          ? normalized.legalName
          : party.legal_name;
        const related = await relatedRows(commandTx, input.envelope.tenantId, [
          party.id,
        ]);
        const currentProfile = profile(party, related.roles, related.contacts);
        const commandRows = await commandTx<UpdateCommandRow[]>`
          SELECT party_id, party_kind, display_name, legal_name, changed, changed_fields
          FROM public.update_party_profile(
        ...
        if (
          !command.changed_fields.every(
            (field) => field === "display_name" || field === "legal_name",
          )
        ) {
          throw new Error(
            "Party update command returned an invalid changed field",
          );
        }
        return {
          status: 200,
          body: Object.freeze({
            partyId: party.id,
            kind: asKind(command.party_kind),
            legalName: command.legal_name,
            roles: currentProfile.roles,
            contacts: currentProfile.contacts,
            changed: command.changed,
            changedFields: Object.freeze(
              command.changed_fields.map(
                (field) => field as "display_name" | "legal_name",
              ),
            ),
          }),
        };
    ...
    return Object.freeze({
      ...
      party: Object.freeze({
        ...
        kind: outcome.body.kind,
        displayName: normalized.displayName,
        legalName: outcome.body.legalName,
        status: "active",
        roles: outcome.body.roles,
        contacts: outcome.body.contacts,
      }),
