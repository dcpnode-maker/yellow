  ...
  async resolve(tx: Tx, raw: IndiaGstAccommodationFinalComponentTaxSemanticRouteInput): Promise<IndiaGstAccommodationFinalComponentTaxSemanticRouteResult> {
    ...
    const roots = await tx<Row[]>`
      SELECT tax.id::text AS tax_id, tax.generation, tax.evidence_hash,
             ...
             valuation.evidence_hash AS current_final_valuation_evidence_hash,
             applicability.evidence_hash AS current_applicability_evidence_hash,
    ...
    if (storedHash(root.final_valuation_evidence_hash, "final valuation evidence hash")
          !== storedHash(root.current_final_valuation_evidence_hash, "current final valuation evidence hash")
        || storedHash(root.quoted_rate_applicability_evidence_hash, "quoted applicability evidence hash")
          !== storedHash(root.current_applicability_evidence_hash, "current applicability evidence hash")) {
      throw new IndiaGstAccommodationFinalComponentTaxSemanticRouteConflictError("final component-tax predecessor evidence is stale");
    }
