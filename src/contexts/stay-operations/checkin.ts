...
async function loadReadiness(
  ...
  if (segment?.sellable_unit_id) {
    const mappings = await tx<Array<{ space_id: string }>>`
      SELECT mapping.space_id
    ...
    if (mappings.length === 1) {
      const conditionRows = lock
        ? await tx<Array<{ condition: string }>>`
            SELECT room_condition AS condition
            FROM public.lock_checkin_room_condition(
              ${input.tenantId}::uuid,
              ${input.propertyNode}::uuid,
              ${mappings[0]!.space_id}::uuid
            )
          `
        : await tx<Array<{ condition: string }>>`
            SELECT condition
            ...
            WHERE tenant_id = ${input.tenantId}::uuid
              AND tenant_id = current_setting('app.tenant_id', true)::uuid
              AND space_id = ${mappings[0]!.space_id}::uuid
          `;
      spaces = Object.freeze([Object.freeze({
        space_id: mappings[0]!.space_id,
        condition: conditionRows[0]?.condition ?? null,
      })]);
    } else {
      spaces = Object.freeze(mappings.map((mapping) => Object.freeze({
  ...
  } else if (roomCondition === "clean") {
    addBlocker(blockers, "room_inspection_required");
  } else if (roomCondition !== null && roomCondition !== "inspected" && !dirtyRoomOverrideRequired) {
