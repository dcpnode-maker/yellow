...
interface SeedOptions {
  ...
  /** Deployment authority for the small identity/configuration preflight only. */
  readonly authorityDatabaseUrl?: string;
...
export async function runScenarioReviewSeed(options: SeedOptions): Promise<ScenarioReviewSeedResult> {
  ...
  const authority = new SQL(options.authorityDatabaseUrl ?? options.databaseUrl, { max: 1 });
  try {
...
// Bun's Windows launcher does not consistently mark TypeScript entrypoints as
// `import.meta.main`.  The explicit deployment flag keeps this reviewed seed
// runnable in the public-demo runtime without changing normal imports/tests.
if (import.meta.main || process.env.YELLOW_RUN_SCENARIO_REVIEW_SEED === "1") {
  ...
  await runScenarioReviewSeed({
    databaseUrl,
    authorityDatabaseUrl: process.env.YELLOW_SCENARIO_REVIEW_AUTHORITY_DATABASE_URL,
  });
}
