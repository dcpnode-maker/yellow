...
export const REVIEW_FOLIO_SERIES_CONFIGURE_PERMISSION = Object.freeze({
  code: "financials.folio-series:configure",
  description: "Configure the property non-fiscal folio numbering series",
});
...
export const DEPARTURE_SERVICE_PERMISSIONS = Object.freeze([
  Object.freeze({
    code: "stay-operations.departure-services:read",
    description: "Read governed departure service coordination work",
  }),
  Object.freeze({
    code: "stay-operations.departure-services:request",
    description: "Propose a governed departure service request",
  }),
  Object.freeze({
    code: "stay-operations.departure-services:confirm",
    description: "Confirm a governed departure service proposal",
  }),
  Object.freeze({
    code: "stay-operations.departure-services:dispatch",
    description: "Assign governed departure service work to active property staff",
  }),
  Object.freeze({
    code: "stay-operations.departure-services:work",
    description: "Start and complete governed departure service work",
  }),
  Object.freeze({
    code: "stay-operations.departure-services:escalate",
    description: "Escalate governed departure service work to a configured duty role",
  }),
] as const);
export const DEPARTURE_SERVICE_ROLE_FIXTURES = Object.freeze([
  Object.freeze({
    slug: "front-desk-cashier",
    ...
    email: "departure-front-desk-cashier@yellow.local",
    displayName: "Synthetic Departure Front Desk Cashier",
    partyDisplayName: "Synthetic Front Desk Cashier",
  }),
  Object.freeze({
    slug: "housekeeping-desk",
    ...
    email: "departure-housekeeping-desk@yellow.local",
    displayName: "Synthetic Departure Housekeeping Desk",
    partyDisplayName: "Synthetic Housekeeping Desk",
  }),
  Object.freeze({
    slug: "housekeeping-team-lead",
    name: "Housekeeping Team Lead",
    permissionCodes: Object.freeze([
      "stay-operations.departure-services:read",
      "stay-operations.departure-services:dispatch",
      "stay-operations.departure-services:work",
    ...
    email: "departure-housekeeping-team-lead@yellow.local",
    displayName: "Synthetic Departure Housekeeping Team Lead",
    partyDisplayName: "Synthetic Housekeeping Team Lead",
  }),
  Object.freeze({
    ...
    email: "departure-assistant-manager@yellow.local",
    displayName: "Synthetic Departure Assistant Manager",
    partyDisplayName: "Synthetic Assistant Manager",
  }),
  Object.freeze({
    slug: "duty-manager",
    name: "Duty Manager",
    ...
    email: "departure-duty-manager@yellow.local",
    displayName: "Synthetic Departure Duty Manager",
    partyDisplayName: "Synthetic Duty Manager",
  }),
] as const);
...
export const REVIEW_PERMISSIONS = Object.freeze([
  ...
  REVIEW_FOLIO_SERIES_CONFIGURE_PERMISSION,
...
const DEPARTURE_ROLE_FIXTURE_UUID = `${TENANT_NAME}/departure-service-roles`;
...
const CHECKIN_EXAMPLES = Object.freeze([
  Object.freeze({ key: "clean", confirmationNo: "ARR-CLEAN", displayName: "Aarav Mehta",
    roomCode: "101", condition: "clean" as const, hasIdentityDocument: true }),
  Object.freeze({ key: "dirty", confirmationNo: "ARR-DIRTY", displayName: "Kavya Iyer",
    roomCode: "102", condition: "dirty" as const, hasIdentityDocument: true }),
  Object.freeze({ key: "unassigned", confirmationNo: "ARR-UNASSIGNED",
    displayName: "Rohan Kulkarni", roomCode: null, condition: null,
    hasIdentityDocument: true }),
  Object.freeze({ key: "identity-gated", confirmationNo: "ARR-IDENTITY", displayName: "Nisha Rao",
...
type DepartureRoleFixture = (typeof DEPARTURE_SERVICE_ROLE_FIXTURES)[number];

async function provisionDepartureRole(
  connection: ReservedSQL,
  role: DepartureRoleFixture,
  password: string,
): Promise<void> {
  const roleId = await uuidV5(SEED_TENANT.id, `${DEPARTURE_ROLE_FIXTURE_UUID}/${role.slug}/role`);
  const roleRows = await connection<RoleRow[]>`
    SELECT id, tenant_id, name
    FROM role
    WHERE id=${roleId}::uuid
       OR (tenant_id=${SEED_TENANT.id}::uuid AND name=${role.name})
    ORDER BY id
  `;
  const expectedRole = { id: roleId, tenant_id: SEED_TENANT.id, name: role.name };
  if (roleRows.length === 0) {
    await connection`
      INSERT INTO role (id, tenant_id, name)
      VALUES (${roleId}::uuid, ${SEED_TENANT.id}::uuid, ${role.name})
    `;
  } else {
    exact(roleRows[0], expectedRole, `Departure role ${role.name}`);
    if (roleRows.length !== 1) throw new Error(`Departure role ${role.name} is ambiguous`);
  }

  const expectedPermissions = [...role.permissionCodes].sort();
  const existingPermissionRows = await connection<Array<{ role_id: string; permission_code: string }>>`
    SELECT role_id, permission_code
    FROM role_permission
    WHERE role_id=${roleId}::uuid
    ORDER BY permission_code
  `;
  for (const permissionCode of expectedPermissions) {
    if (!existingPermissionRows.some(({ permission_code }) => permission_code === permissionCode)) {
      await connection`
  ...
  const permissionRows = await connection<Array<{ role_id: string; permission_code: string }>>`
    SELECT role_id, permission_code
    FROM role_permission
    WHERE role_id=${roleId}::uuid
    ORDER BY permission_code
  `;
  const actualPermissions = permissionRows.map(({ permission_code }) => permission_code);
  if (stableJson(actualPermissions) !== stableJson(expectedPermissions)) {
    throw new Error(`Departure role ${role.name} permissions collide with non-canonical local-review data`);
  }

  const userId = await uuidV5(SEED_TENANT.id, `${DEPARTURE_ROLE_FIXTURE_UUID}/${role.slug}/user`);
  await provisionReviewUser(connection, password, {
    id: userId,
    email: role.email,
    displayName: role.displayName,
    label: `Departure ${role.name} user`,
  });
  const grants = await connection<Array<{ tenant_id: string; user_id: string; role_id: string; scope_node: string }>>`
    SELECT tenant_id, user_id, role_id, scope_node
    FROM user_role
    WHERE user_id=${userId}::uuid
    ORDER BY role_id, scope_node
  `;
  const expectedGrant = {
    tenant_id: SEED_TENANT.id, user_id: userId, role_id: roleId, scope_node: SEED_PROPERTY.id,
  };
  if (grants.length === 0) {
    await connection`
    ...
    if (grants.length !== 1) throw new Error(`Departure ${role.name} user role grants are ambiguous`);
    exact(grants[0], expectedGrant, `Departure ${role.name} user role grant`);
  }

  const partyId = await uuidV5(SEED_TENANT.id, `${DEPARTURE_ROLE_FIXTURE_UUID}/${role.slug}/party`);
  const attrs = Object.freeze({
    source: "local-review",
    synthetic: true,
    departure_service_fixture: role.slug,
  });
  const partyRows = await connection<Array<Record<string, unknown>>>`
    SELECT id, tenant_id, kind, display_name, legal_name, attrs, vip_code, status, merged_into
    FROM party
    WHERE id=${partyId}::uuid
       OR (tenant_id=${SEED_TENANT.id}::uuid AND display_name=${role.partyDisplayName})
    ORDER BY id
    FOR UPDATE
  `;
  const expectedParty = {
    id: partyId, tenant_id: SEED_TENANT.id, kind: "person", display_name: role.partyDisplayName,
    legal_name: role.partyDisplayName, attrs, vip_code: null, status: "active", merged_into: null,
  };
  if (partyRows.length === 0) {
    await connection`
      INSERT INTO party (id, tenant_id, kind, display_name, legal_name, attrs, status)
      VALUES (${partyId}::uuid, ${SEED_TENANT.id}::uuid, 'person', ${role.partyDisplayName},
        ${role.partyDisplayName}, ${JSON.stringify(attrs)}::text::jsonb, 'active')
    `;
  } else {
    exact(partyRows[0], expectedParty, `Departure ${role.name} staff Party`);
    if (partyRows.length !== 1) throw new Error(`Departure ${role.name} staff Party is ambiguous`);
  }

  const partyRoles = await connection<Array<Record<string, unknown>>>`
    SELECT tenant_id, party_id, role, detail
    FROM party_role
    WHERE party_id=${partyId}::uuid
    ORDER BY role
  `;
  const expectedPartyRole = { tenant_id: SEED_TENANT.id, party_id: partyId, role: "staff", detail: attrs };
  if (partyRoles.length === 0) {
    await connection`
      INSERT INTO party_role (tenant_id, party_id, role, detail)
      VALUES (${SEED_TENANT.id}::uuid, ${partyId}::uuid, 'staff', ${JSON.stringify(attrs)}::text::jsonb)
    `;
  } else {
    if (partyRoles.length !== 1) throw new Error(`Departure ${role.name} staff Party roles are ambiguous`);
    exact(partyRoles[0], expectedPartyRole, `Departure ${role.name} staff Party role`);
  }
}

async function provisionDepartureRoleFixtures(connection: ReservedSQL, password: string): Promise<void> {
  await connection`SELECT pg_advisory_xact_lock(hashtextextended('yellow.local.review.seed.departure-service-roles', 0))`;
  for (const permission of DEPARTURE_SERVICE_PERMISSIONS) {
    ...
    if (rows.length === 0) {
      ...
      if (rows.length !== 1) throw new Error(`Departure permission ${permission.code} is ambiguous`);
      exact(rows[0], permission, `Departure permission ${permission.code}`);
    }
  }
  for (const role of DEPARTURE_SERVICE_ROLE_FIXTURES) {
    await provisionDepartureRole(connection, role, password);
  }
}
...
async function provisionIdentity(
  ...
  for (const permission of REVIEW_PERMISSIONS) {
    if (permission.code === REVIEW_DISCREPANCY_CARRY_PERMISSION.code ||
      ...
      permission.code === REVIEW_FOLIO_SERIES_CONFIGURE_PERMISSION.code ||
  ...
  `;
  await provisionDepartureRoleFixtures(connection, password);
}
...
async function provisionVehicleExamples(
  ...
  const parkingFolioId = await uuidV5(
    SEED_TENANT.id,
    `${REVIEW_VEHICLE_FIXTURE_UUID}/parking-primary-folio`,
  );
  const parkingSources = await connection<Array<{
    party_id: string; rate_plan_id: string;
  }>>`
    SELECT reservation.primary_party AS party_id, segment.rate_plan_id
      FROM reservation
  ...
  if (!parkingSource || parkingSources.length !== 1) {
    throw new Error("Local-review parking vehicle source stay is absent or inconsistent");
  }
  const parkingInventoryRows = await connection<Array<{
    unit_type_id: string; sellable_unit_id: string; space_id: string;
  }>>`
    SELECT sellable.unit_type_id, sellable.id AS sellable_unit_id, assignment.space_id
      FROM sellable_unit AS sellable
      JOIN unit_type
        ON unit_type.tenant_id=sellable.tenant_id AND unit_type.id=sellable.unit_type_id
      JOIN sellable_unit_space AS assignment
        ON assignment.tenant_id=sellable.tenant_id AND assignment.sellable_unit_id=sellable.id
      JOIN space ON space.tenant_id=assignment.tenant_id AND space.id=assignment.space_id
     WHERE sellable.tenant_id=${SEED_TENANT.id}::uuid
       ...
       AND space.code='203'
     ORDER BY sellable.id
  `;
  const parkingInventory = parkingInventoryRows[0];
  if (!parkingInventory || parkingInventoryRows.length !== 1) {
    throw new Error("Local-review parking vehicle inventory source is absent or inconsistent");
  }
  ...
  if (parkingReservations.length === 0) {
    ...
    await connection`INSERT INTO reservation_segment(
      ...
      ${parkingInventory.unit_type_id}::uuid,${parkingInventory.sellable_unit_id}::uuid,
      -- Keep the active synthetic cashier stay one complete day beyond the
      -- thirty-day availability-review horizon.  A same-day clock boundary must
      -- never make the seeded occupied room appear sellable before the test runs.
      tstzrange(transaction_timestamp()-interval '1 day',transaction_timestamp()+interval '31 days','[)'),
  ...
  const parkingGuests = await connection<Array<Record<string, unknown>>>`
    SELECT tenant_id, reservation_id, party_id, role, share_pct::text
      FROM reservation_guest
     WHERE tenant_id=${SEED_TENANT.id}::uuid
       AND reservation_id=${parkingReservationId}::uuid
       AND party_id=${parkingSource.party_id}::uuid
     ORDER BY role
  `;
  const expectedParkingGuest = { tenant_id: SEED_TENANT.id, reservation_id: parkingReservationId,
    party_id: parkingSource.party_id, role: "primary", share_pct: null };
  if (parkingGuests.length === 0) {
    await connection`INSERT INTO reservation_guest (tenant_id, reservation_id, party_id, role)
      VALUES (${SEED_TENANT.id}::uuid, ${parkingReservationId}::uuid, ${parkingSource.party_id}::uuid, 'primary')`;
  } else {
    exact(parkingGuests[0], expectedParkingGuest, "Local-review parking assignment reservation guest");
    if (parkingGuests.length !== 1) throw new Error("Local-review parking assignment reservation guest is ambiguous");
  }

  let parkingOccupancies = await connection<Array<Record<string, unknown>>>`
    SELECT occupancy.id, occupancy.tenant_id, occupancy.space_id, occupancy.slot_ref,
           occupancy.slot_kind, occupancy.exclusive, occupancy.claim::text,
           occupancy.period=segment.period AS exact_period
      FROM space_occupancy AS occupancy
      JOIN reservation_segment AS segment
        ON segment.tenant_id=occupancy.tenant_id AND segment.id=occupancy.slot_ref
     WHERE occupancy.tenant_id=${SEED_TENANT.id}::uuid
       AND occupancy.slot_ref=${parkingSegmentId}::uuid
     ORDER BY occupancy.id
  `;
  if (parkingOccupancies.length === 0) {
    await connection`SELECT record_occupancy(
      ${SEED_TENANT.id}::uuid, ${parkingInventory.space_id}::uuid,
      (SELECT period FROM reservation_segment
        WHERE tenant_id=${SEED_TENANT.id}::uuid AND id=${parkingSegmentId}::uuid),
      ${parkingSegmentId}::uuid, 'segment', true)`;
    parkingOccupancies = await connection<Array<Record<string, unknown>>>`
      SELECT occupancy.id, occupancy.tenant_id, occupancy.space_id, occupancy.slot_ref,
             occupancy.slot_kind, occupancy.exclusive, occupancy.claim::text,
             occupancy.period=segment.period AS exact_period
        FROM space_occupancy AS occupancy
  ...
  const parkingOccupancy = parkingOccupancies[0];
  if (!parkingOccupancy) throw new Error("Local-review parking assignment occupancy is absent");
  exact(parkingOccupancy, { id: parkingOccupancy.id, tenant_id: SEED_TENANT.id,
    space_id: parkingInventory.space_id, slot_ref: parkingSegmentId, slot_kind: "segment",
    exclusive: true, claim: "[0,)", exact_period: true }, "Local-review parking assignment occupancy");
  if (parkingOccupancies.length !== 1) throw new Error("Local-review parking assignment occupancy is ambiguous");

  const parkingAccountRows = await connection<Array<Record<string, unknown>>>`
    SELECT id, tenant_id, property_node, role, party_id, name, currency::text, status
      FROM account
     WHERE id=${housekeepingExamples.departureAccountId}::uuid
     FOR UPDATE
  `;
  const parkingAccount = parkingAccountRows[0];
  if (!parkingAccount || parkingAccountRows.length !== 1 || parkingAccount.tenant_id !== SEED_TENANT.id ||
      parkingAccount.property_node !== SEED_PROPERTY.id || parkingAccount.role !== "guest" ||
      parkingAccount.party_id !== parkingSource.party_id || parkingAccount.currency !== SEED_PROPERTY.currency ||
      parkingAccount.status !== "open") {
    throw new Error("Local-review parking assignment guest account is absent or inconsistent");
  }
  const parkingFolioNo = "PARKING-REVIEW-1";
  const parkingFolios = await connection<Array<Record<string, unknown>>>`
    SELECT folio.id, folio.tenant_id, folio.account_id, folio.reservation_id,
           folio.folio_no, folio.window_no, folio.name, folio.status,
           COALESCE(balance.balance_minor, 0)::text AS balance_minor,
           COALESCE(balance.lines, 0)::int AS posting_lines
      ...
      LEFT JOIN folio_balance AS balance
        ON balance.tenant_id=folio.tenant_id AND balance.folio_id=folio.id
     WHERE folio.id=${parkingFolioId}::uuid
        OR (folio.tenant_id=${SEED_TENANT.id}::uuid
          AND (folio.folio_no=${parkingFolioNo}
            OR (folio.reservation_id=${parkingReservationId}::uuid AND folio.window_no=1)))
     ORDER BY folio.id
     FOR UPDATE OF folio
  `;
  const expectedParkingFolio = { id: parkingFolioId, tenant_id: SEED_TENANT.id,
    account_id: housekeepingExamples.departureAccountId, reservation_id: parkingReservationId,
    folio_no: parkingFolioNo, window_no: 1, name: "Primary", status: "open",
    balance_minor: "0", posting_lines: 0 };
  if (parkingFolios.length === 0) {
    await connection`INSERT INTO folio (id, tenant_id, account_id, reservation_id, folio_no, window_no, name, status)
      VALUES (${parkingFolioId}::uuid, ${SEED_TENANT.id}::uuid,
        ${housekeepingExamples.departureAccountId}::uuid, ${parkingReservationId}::uuid,
        ${parkingFolioNo}, 1, 'Primary', 'open')`;
  } else {
    exact(parkingFolios[0], expectedParkingFolio, "Local-review parking assignment primary folio");
    if (parkingFolios.length !== 1) throw new Error("Local-review parking assignment primary folio is ambiguous");
  }
