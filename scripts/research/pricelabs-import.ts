...
/** Read-only verification shared by POSIX output and the separately ACL-bound Windows writer. */
export function preparePriceLabsArchive(archive: string) {
  const verified = readVerifiedArchive(archive);
  ...
  const archiveFiles = [
    { path: "manifest.json", bytes: verified.manifestBytes, sha256: staging.provenance.manifestSha256, manifestListed: false },
    ...verified.manifest.files.map(file => ({ path: file.path, bytes: verified.contents.get(file.path)!, sha256: file.sha256, manifestListed: true })),
  ];
  return { root: verified.root, staging, receipt, archiveFiles };
}
...
export function importPriceLabsArchive(options: { archive: string; output: string }): PriceLabsImportReceipt {
  ...
  const { root, staging, receipt } = preparePriceLabsArchive(options.archive);
  const output = canonicalOutputPath(options.output);
  if (isWithin(root, output) || isWithin(output, root)) fail("archive_output_overlap");
  let created = false;
  try {
