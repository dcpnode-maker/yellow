...
composes exact validation-compatible pre-document evidence, independently validates
child hashes/lineage/state, and returns only frozen false readiness with no actions
and the explicit origin, legal-number-format, and series blockers. It has no
document, numbering, series, provider, submission, IRN/QR, write, API/UI, or phase
completion authority. Intentional-red, hostile pure, and real-PostgreSQL census
