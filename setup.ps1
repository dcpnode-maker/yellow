...
try {
    ...
    $tables = $tables.Trim()
    if ($tables -ne '83') { throw "yellow_test has $tables public tables; expected 83 (80 baseline + 2 kernel consumer + schema_migration)." }
    Write-Host 'yellow_test tables: 83 (80 baseline + 2 kernel consumer + schema_migration)'
