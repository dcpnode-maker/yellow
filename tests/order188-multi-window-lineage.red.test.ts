...
describe("Order 188 P0 typed immutable folio-transfer lineage", () => {
  test("migration 0020 owns typed root lineage and the bounded transfer capability", () => {
    ...
    expect(sql).toMatch(/FOREIGN KEY\s*\(tenant_id,\s*folio_transfer_root_line_id\)\s+REFERENCES\s+posting_line\s*\(tenant_id,\s*id\)/i);
    expect(sql).toMatch(/CREATE INDEX\s+\w+\s+ON\s+posting_line\s*\(tenant_id,\s*folio_transfer_root_line_id\)\s+WHERE\s+folio_transfer_root_line_id IS NOT NULL/i);
    ...
    expect(sql).toMatch(/session_user\s*<>\s*'app_role'/i);
    expect(sql).toMatch(/current_setting\s*\(\s*'app\.tenant_id'\s*,\s*true\s*\)/i);
    ...
    expect(sql).toMatch(/INSERT INTO journal[\s\S]*'transfer'/i);
    expect(sql).toMatch(/INSERT INTO posting_line[\s\S]*folio_transfer_root_line_id/i);
