...
databaseDescribe("Order 048 operator inventory management", () => {
  ...
  test("P6/P7: assets expose one themed inventory UI and login carries exact scopes", async () => {
    ...
    expect((await tokens.verify(accessToken))?.scp).toBe(
      "inventory.availability:read inventory.configuration:read inventory.configuration:write",
    );
  });
});
