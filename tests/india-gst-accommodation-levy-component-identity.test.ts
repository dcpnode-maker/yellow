...
describe("Order 337: India GST accommodation numeric component-rate split",()=>{
  ...
  test("preserves historical and active aggregate schedules exactly once while splitting every even rate",async()=>{
    const expectedDual={
      "2025-09-21":{cgst_sgst:[[{identity:"cgst",rate:0.06,rateBasisPoints:600},{identity:"sgst",rate:0.06,rateBasisPoints:600}],[{identity:"cgst",rate:0.09,rateBasisPoints:900},{identity:"sgst",rate:0.09,rateBasisPoints:900}]],cgst_utgst:[[{identity:"cgst",rate:0.06,rateBasisPoints:600},{identity:"utgst",rate:0.06,rateBasisPoints:600}],[{identity:"cgst",rate:0.09,rateBasisPoints:900},{identity:"utgst",rate:0.09,rateBasisPoints:900}]]},
      "2025-09-22":{cgst_sgst:[[{identity:"cgst",rate:0.025,rateBasisPoints:250},{identity:"sgst",rate:0.025,rateBasisPoints:250}],[{identity:"cgst",rate:0.09,rateBasisPoints:900},{identity:"sgst",rate:0.09,rateBasisPoints:900}]],cgst_utgst:[[{identity:"cgst",rate:0.025,rateBasisPoints:250},{identity:"utgst",rate:0.025,rateBasisPoints:250}],[{identity:"cgst",rate:0.09,rateBasisPoints:900},{identity:"utgst",rate:0.09,rateBasisPoints:900}]]},
    } as const;
    for(const day of ["2025-09-21","2025-09-22"] as const){for(const family of ["igst","cgst_sgst","cgst_utgst"] as const){const input=complete(await historical(day) as Mutable,family),componentIdentity=derive(input),actual=deriveRates({...input,componentIdentity}),text=JSON.stringify(actual);expect(actual.componentRateSlabs.map((v:Mutable)=>({uptoMinor:v.uptoMinor,rate:v.aggregateRate,itcEligible:v.itcEligible}))).toEqual(input.levyInputBundle.gstRoomSlabs);for(const slab of actual.componentRateSlabs){expect(slab.components.reduce((sum:number,v:Mutable)=>sum+v.rateBasisPoints,0)).toBe(slab.aggregateRateBasisPoints);}if(family!=="igst")expect(actual.componentRateSlabs.map((slab:Mutable)=>slab.components)).toEqual(expectedDual[day][family]);expect(text.match(/componentRateSlabs/g)).toHaveLength(1);expect(text).not.toMatch(/taxableValue|taxMinor|taxAmount|rounding|residual|posting|document|irp/i);}}
  });
