...
--
-- Name: record_india_gst_accommodation_final_valuation(uuid, uuid, uuid, uuid, uuid, uuid, uuid, uuid, text, text, text, text, uuid, text, uuid, text[], text[], uuid[], text[], text[], text[], integer[], date[], bigint[], bigint[]); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.record_india_gst_accommodation_final_valuation(p_tenant uuid, p_property uuid, p_reservation uuid, p_folio uuid, p_buyer uuid, p_attribution uuid, p_request uuid, p_actor uuid, p_disposition text, p_order341_hash text, p_request_hash text, p_evidence_hash text, p_expected uuid, p_expected_hash text, p_approval uuid, p_ordinary_hashes text[], p_manual_reasons text[], p_source_ids uuid[], p_source_kinds text[], p_source_hashes text[], p_source_eligibility text[], p_room_ordinals integer[], p_room_dates date[], p_room_weights bigint[], p_allocations bigint[]) RETURNS TABLE(valuation_id uuid, generation integer, disposition text, transaction_value_minor bigint, evidence_hash text)
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'pg_catalog', 'public'
    AS $_$
DECLARE
...
CREATE TABLE public.india_gst_accommodation_final_valuation (
    ...
    CONSTRAINT india_gst_final_valuation_shape_ck CHECK (((((generation = 0) AND (supersedes_valuation_id IS NULL)) OR ((generation > 0) AND (supersedes_valuation_id IS NOT NULL))) AND (((disposition = 'ordinary_final'::text) AND (transaction_value_minor IS NOT NULL) AND (cardinality(ordinary_evidence_hashes) = 5) AND (cardinality(manual_reasons) = 0)) OR ((disposition = 'manual_valuation_required'::text) AND (transaction_value_minor IS NULL) AND (cardinality(manual_reasons) > 0))) AND (NOT (ordinary_evidence_hashes && manual_reasons))))
);
