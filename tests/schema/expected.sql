...
DECLARE
  ...
  v_expected_lower integer; v_expected_content_hash text; v_expected_content jsonb; v_extension public.extension%ROWTYPE; v_replay boolean:=false;
...
BEGIN
  ...
  IF (p_calendar_authority IS NULL)<>(v_calendar_count=0) OR pg_catalog.cardinality(p_calendar_states)<>v_calendar_count OR v_calendar_count>366 THEN RAISE EXCEPTION USING ERRCODE='22023',MESSAGE='calendar evidence shape is invalid'; END IF;
  IF v_calendar_count>0 THEN
    IF v_calendar_count<4 OR p_calendar_source_hash!~'^[0-9a-f]{64}$' OR p_calendar_through IS DISTINCT FROM p_calendar_dates[v_calendar_count] THEN RAISE EXCEPTION USING ERRCODE='22023',MESSAGE='calendar evidence is incomplete'; END IF;
    FOR v_i IN 1..v_calendar_count LOOP IF p_calendar_dates[v_i]<>p_rate_change_date+v_i OR p_calendar_states[v_i] NOT IN ('working','non_working') THEN RAISE EXCEPTION USING ERRCODE='22023',MESSAGE='calendar evidence is not dense'; END IF; IF p_calendar_states[v_i]='working' THEN v_working:=v_working+1; END IF; END LOOP;
    ...
    IF NOT (v_payment.supplier_books_entry_date<=p_rate_change_date AND v_payment.supplier_bank_credit_date>p_rate_change_date)
       OR p_payment_date<>least(v_payment.supplier_books_entry_date,p_calendar_dates[(SELECT min(i) FROM generate_subscripts(p_calendar_states,1)i WHERE p_calendar_states[i]='working' AND (SELECT count(*) FROM generate_subscripts(p_calendar_states,1)j WHERE j<=i AND p_calendar_states[j]='working')=4)]) THEN RAISE EXCEPTION USING ERRCODE='22023',MESSAGE='calendar evidence does not derive governed payment date'; END IF;
  ...
  IF v_replay THEN
    IF v_existing.property_node IS DISTINCT FROM p_property OR v_existing.reservation_id IS DISTINCT FROM p_reservation OR v_existing.folio_id IS DISTINCT FROM p_folio OR v_existing.reservation_lineage_id IS DISTINCT FROM p_lineage OR v_existing.attribution_id IS DISTINCT FROM p_attribution OR v_existing.service_provision_snapshot_id IS DISTINCT FROM p_service_snapshot OR v_existing.payment_receipt_snapshot_id IS DISTINCT FROM p_payment_snapshot OR v_existing.invoice_issue_snapshot_id IS DISTINCT FROM p_invoice_snapshot OR v_existing.family_jurisdiction_extension_id IS DISTINCT FROM p_family_jurisdiction_extension OR v_existing.classification_id IS DISTINCT FROM p_classification OR v_existing.supplier_service_location_id IS DISTINCT FROM p_supplier_service_location OR v_existing.supplier_sez_status_id IS DISTINCT FROM p_supplier_sez_status OR v_existing.recipient_sez_status_id IS DISTINCT FROM p_recipient_sez_status OR v_existing.recipient_party_id IS DISTINCT FROM p_recipient_party OR v_existing.final_valuation_id IS DISTINCT FROM v_valuation.id OR v_existing.actor_id IS DISTINCT FROM p_actor OR v_existing.section14_case IS DISTINCT FROM p_section14_case OR v_existing.service_provision_date IS DISTINCT FROM p_service_date OR v_existing.invoice_issue_date IS DISTINCT FROM p_invoice_date OR v_existing.payment_receipt_date IS DISTINCT FROM p_payment_date OR v_existing.rate_change_date IS DISTINCT FROM p_rate_change_date OR v_existing.time_of_supply_date IS DISTINCT FROM p_time_of_supply_date OR v_existing.selected_version_side IS DISTINCT FROM p_selected_side OR v_existing.selected_extension_id IS DISTINCT FROM p_selected_extension OR v_existing.selected_extension_version IS DISTINCT FROM p_selected_version OR v_existing.selected_extension_status IS DISTINCT FROM p_selected_status OR v_existing.selected_content_hash IS DISTINCT FROM p_selected_content_hash OR v_existing.selected_effective_from IS DISTINCT FROM p_selected_from OR v_existing.selected_effective_to IS DISTINCT FROM p_selected_to OR v_existing.component_family IS DISTINCT FROM p_component_family OR v_existing.section14_evidence_hash IS DISTINCT FROM p_section14_hash OR v_existing.levy_component_identity_evidence_hash IS DISTINCT FROM p_levy_hash OR v_existing.reservation_lineage_evidence_hash IS DISTINCT FROM p_lineage_hash OR v_existing.attribution_snapshot_evidence_hash IS DISTINCT FROM p_attribution_hash OR v_existing.evidence_hash IS DISTINCT FROM p_evidence_hash OR v_existing.calendar_authority_id IS DISTINCT FROM p_calendar_authority OR v_existing.calendar_source_digest_sha256 IS DISTINCT FROM p_calendar_source_hash OR v_existing.calendar_through_date IS DISTINCT FROM p_calendar_through OR v_existing.calendar_dates IS DISTINCT FROM p_calendar_dates OR v_existing.calendar_states IS DISTINCT FROM p_calendar_states
      OR (SELECT array_agg(ordinal ORDER BY ordinal) FROM public.india_gst_accommodation_quoted_rate_applicability_room_night WHERE tenant_id=p_tenant AND applicability_id=v_existing.id) IS DISTINCT FROM p_night_ordinals
      OR (SELECT array_agg(business_date ORDER BY ordinal) FROM public.india_gst_accommodation_quoted_rate_applicability_room_night WHERE tenant_id=p_tenant AND applicability_id=v_existing.id) IS DISTINCT FROM p_night_dates
      OR (SELECT array_agg(quoted_amount_minor ORDER BY ordinal) FROM public.india_gst_accommodation_quoted_rate_applicability_room_night WHERE tenant_id=p_tenant AND applicability_id=v_existing.id) IS DISTINCT FROM p_night_amounts
      OR (SELECT array_agg(slab_upto_minor ORDER BY ordinal) FROM public.india_gst_accommodation_quoted_rate_applicability_room_night WHERE tenant_id=p_tenant AND applicability_id=v_existing.id) IS DISTINCT FROM p_slab_upto
      OR (SELECT array_agg(aggregate_rate_basis_points ORDER BY ordinal) FROM public.india_gst_accommodation_quoted_rate_applicability_room_night WHERE tenant_id=p_tenant AND applicability_id=v_existing.id) IS DISTINCT FROM p_aggregate_bps
      OR (SELECT array_agg(itc_eligible ORDER BY ordinal) FROM public.india_gst_accommodation_quoted_rate_applicability_room_night WHERE tenant_id=p_tenant AND applicability_id=v_existing.id) IS DISTINCT FROM p_itc_eligible
      OR (SELECT array_agg(room_night_ordinal ORDER BY room_night_ordinal,component_ordinal) FROM public.india_gst_accommodation_quoted_rate_component WHERE tenant_id=p_tenant AND applicability_id=v_existing.id) IS DISTINCT FROM p_component_night_ordinals
      OR (SELECT array_agg(component_ordinal ORDER BY room_night_ordinal,component_ordinal) FROM public.india_gst_accommodation_quoted_rate_component WHERE tenant_id=p_tenant AND applicability_id=v_existing.id) IS DISTINCT FROM p_component_ordinals
      OR (SELECT array_agg(component_identity ORDER BY room_night_ordinal,component_ordinal) FROM public.india_gst_accommodation_quoted_rate_component WHERE tenant_id=p_tenant AND applicability_id=v_existing.id) IS DISTINCT FROM p_component_identities
      OR (SELECT array_agg(rate_basis_points ORDER BY room_night_ordinal,component_ordinal) FROM public.india_gst_accommodation_quoted_rate_component WHERE tenant_id=p_tenant AND applicability_id=v_existing.id) IS DISTINCT FROM p_component_bps THEN RAISE EXCEPTION USING ERRCODE='23505',MESSAGE='quoted applicability request has divergent evidence'; END IF;
