...
describe("Order 197 operator cashier workbench", () => {
  ...
  test("Order 461: Cashiers composes a bounded in-house billing search with the existing folio workspace", () => {
    expect(html).toContain('id="cashier-billing-desk"');
    expect(html).toContain('id="cashier-billing-search-form"');
    expect(html).toContain('id="cashier-billing-results" aria-live="polite"');
    expect(script).toContain('reservation-board?status=in_house&limit=100');
    expect(script).toContain('async function openCashierBillingFolio');
    expect(script).toContain('This stay has no existing folio window. Open one only through the explicit reservation command.');
    expect(script).toContain('canonicalFolioPath(property, primary.folioId)');
    expect(script).toContain('void loadFolioWorkspace(primary.folioId)');
  });

  test("Order 462: a client website preview remains separate from the Yellow operator workspace", () => {
    expect(html).toContain('id="client-website-preview"');
    expect(html).toContain('href="/client/locanda-homes"');
    expect(script).toContain('function syncClientWebsitePreview()');
    expect(script).toContain('/locanda|jareed/i');
    expect(script).toContain('"Open Locanda Homes preview"');
    expect(script).toContain('"Preview this property’s guest website"');
  });
