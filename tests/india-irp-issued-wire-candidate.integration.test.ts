...
databaseDescribe("Order440 genuine issued-document wire candidate", () => {
  ...
  beforeAll(async () => {
    ...
    `;
    if (!identity || !identity.database_name.startsWith("yellow_order440_") || identity.migrations !== 77) {
      throw new Error("Order440 proof requires an isolated synthetic 77-migration database");
    }
  });
