...
describe("Order 438 immutable release and local-review contracts", () => {
  test("publishes exact green-main SHA images without deploying a cloud target", async () => {
    ...
    expect(workflow).toContain("MIGRATION_FRONTIER: '92'");
  ...
  test("the one-command local launcher proves exact revision, readiness and login", async () => {
    ...
    expect(launcher).toContain("body.build?.expectedMigrationFrontier !== 92");
    ...
    expect(setup).toContain("expected 129 after migrations 1-92");
    expect(windowsSetup).toContain("expected 129 after migrations 1-92");
