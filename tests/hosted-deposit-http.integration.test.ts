...
const OPERATOR_ACTOR = "33333333-3333-4333-8333-333333333333";
const OPERATOR_PROPERTY = "44444444-4444-4444-8444-444444444444";
const OPERATOR_FOLIO = "55555555-5555-4555-8555-555555555555";

function operatorWithHosted(hosted?: unknown): OperatorHttpApi {
  return new OperatorHttpApi(
    {} as never, undefined, undefined, undefined, undefined, undefined, undefined,
    undefined, undefined, undefined, undefined, undefined, undefined, undefined,
    undefined, undefined, undefined, undefined, undefined, undefined, undefined,
    undefined, undefined, undefined, undefined, undefined, hosted as never,
  );
}

function operatorContext(request: Request, scopes: readonly string[], granted = true): TenantRequestContext {
  const tx = (async () => granted
    ? [{ id: OPERATOR_PROPERTY, name: "Order 578", timezone: "UTC", currency: "INR" }]
    : []) as unknown as Tx;
  return { request, tenantId: OPERATOR_TENANT,
    identity: { tenantId: OPERATOR_TENANT, actorId: OPERATOR_ACTOR, scopes }, tx };
}
...
describe("Order578 hosted-deposit read HTTP boundary", () => {
  test("mounts the exact folio-scoped GET beside the existing POST", async () => {
    const source = await Bun.file(new URL("../src/app.ts", import.meta.url)).text();
    expect(source).toContain('.get("/api/v1/properties/:property/folios/:reference/hosted-deposits"');
    expect(source).toContain("operator.hostedDepositWorkbench(");
  });

  test("composes and dispatches the hosted-deposit GET beside the existing statement GET", async () => {
    const calls: unknown[] = [];
    const operator = new Proxy({
      async hostedDepositWorkbench(_context: TenantRequestContext, property: string, folio: string) {
        calls.push(["deposit", property, folio]);
        return Response.json({ route: "deposit", property, folio });
      },
      async folioStatement(_context: TenantRequestContext, property: string, folio: string) {
        calls.push(["statement", property, folio]);
        return Response.json({ route: "statement", property, folio });
      },
      failure(_request: Request, error: unknown) {
        return Response.json({ error: error instanceof Error ? error.message : String(error) }, { status: 500 });
      },
      unauthorized() { return new Response("unauthorized", { status: 401 }); },
    }, { get(target, property, receiver) {
      return Reflect.has(target, property)
        ? Reflect.get(target, property, receiver)
        : () => new Response("unused", { status: 501 });
    } }) as unknown as OperatorHttpApi;
    const database = { async withTenantTransaction<T>(_tenantId: string, operation: (tx: Tx) => Promise<T>) {
      return operation((async () => []) as unknown as Tx);
    } } as unknown as Database;
    const tenantResolver = { async resolve() {
      return { tenantId: OPERATOR_TENANT, actorId: OPERATOR_ACTOR, scopes: [] };
    } } satisfies TenantResolver;
    const app = createApp({ operatorApi: operator, database, tenantResolver });
    const deposit = await app.handle(new Request(
      `http://yellow.test/api/v1/properties/${OPERATOR_PROPERTY}/folios/${OPERATOR_FOLIO}/hosted-deposits`,
    ));
    const statement = await app.handle(new Request(
      `http://yellow.test/api/v1/properties/${OPERATOR_PROPERTY}/folios/${OPERATOR_FOLIO}/statement`,
    ));
    expect(deposit.status).toBe(200);
    expect(await deposit.json()).toEqual({ route: "deposit", property: OPERATOR_PROPERTY, folio: OPERATOR_FOLIO });
    expect(statement.status).toBe(200);
    expect(await statement.json()).toEqual({ route: "statement", property: OPERATOR_PROPERTY, folio: OPERATOR_FOLIO });
    expect(calls).toEqual([
      ["deposit", OPERATOR_PROPERTY, OPERATOR_FOLIO],
      ["statement", OPERATOR_PROPERTY, OPERATOR_FOLIO],
    ]);
  });

  test("requires the existing payment-read scope and exact property grant before the service", async () => {
    let calls = 0;
    const hosted = { async workbenchForFolio() { calls += 1; throw new Error("must not be called"); } };
    const operator = operatorWithHosted(hosted);
    const request = new Request(`http://yellow.test/api/v1/properties/${OPERATOR_PROPERTY}/folios/${OPERATOR_FOLIO}/hosted-deposits`);
    expect((await operator.hostedDepositWorkbench(operatorContext(request, []), OPERATOR_PROPERTY, OPERATOR_FOLIO)).status).toBe(403);
    expect((await operator.hostedDepositWorkbench(
      operatorContext(request, ["financials.payments:read"], false), OPERATOR_PROPERTY, OPERATOR_FOLIO,
    )).status).toBe(403);
    expect(calls).toBe(0);
  });

  test("rejects hostile identities and query widening before the read service", async () => {
    let calls = 0;
    const operator = operatorWithHosted({ async workbenchForFolio() { calls += 1; return {}; } });
    for (const [url, property, folio] of [
      [`http://yellow.test/api/v1/properties/${OPERATOR_PROPERTY}/folios/${OPERATOR_FOLIO}/hosted-deposits?limit=1`, OPERATOR_PROPERTY, OPERATOR_FOLIO],
      ["http://yellow.test/read", "not-a-uuid", OPERATOR_FOLIO],
      ["http://yellow.test/read", OPERATOR_PROPERTY, "not-a-uuid"],
    ] as const) {
      const response = await operator.hostedDepositWorkbench(
        operatorContext(new Request(url), ["financials.payments:read"]), property, folio,
      );
      expect(response.status).toBe(400);
    }
    expect(calls).toBe(0);
  ...
  test("returns only canonical service JSON and remains unavailable without the hosted service", async () => {
    const calls: unknown[] = [];
    const result = { propertyNode: OPERATOR_PROPERTY, folioId: OPERATOR_FOLIO,
      deposits: [{ requestId: "66666666-6666-4666-8666-666666666666", operationId: "22222222-2222-4222-8222-222222222222",
        tenantId: OPERATOR_TENANT, propertyNode: OPERATOR_PROPERTY, propertyName: "Order 578", folioId: OPERATOR_FOLIO,
        folioReference: "FOL-578", amountMinor: "5000", currency: "INR", generation: 1,
        expiresAt: "2026-09-22T00:00:00.000Z", state: "captured", capturedMinor: "5000",
        appliedMinor: "2000", remainingMinor: "3000" }],
      instruments: [{ instrumentId: "77777777-7777-4777-8777-777777777777", kind: "card_network_token",
        brand: "Visa", last4: "5781", expiry: "12/30", psp: "provider" }] };
    const operator = operatorWithHosted({ async workbenchForFolio(input: unknown) { calls.push(input); return result; } });
    const request = new Request(`http://yellow.test/api/v1/properties/${OPERATOR_PROPERTY}/folios/${OPERATOR_FOLIO}/hosted-deposits`);
    const response = await operator.hostedDepositWorkbench(
      operatorContext(request, ["financials.payments:read"]), OPERATOR_PROPERTY, OPERATOR_FOLIO,
    );
    expect(response.status).toBe(200);
    expect(response.headers.get("cache-control")).toBe("no-store");
    expect(await response.json()).toEqual(result);
    expect(calls).toEqual([{ tenantId: OPERATOR_TENANT, propertyNode: OPERATOR_PROPERTY, folioId: OPERATOR_FOLIO }]);

    const unavailable = operatorWithHosted();
    expect((await unavailable.hostedDepositWorkbench(
      operatorContext(request, ["financials.payments:read"]), OPERATOR_PROPERTY, OPERATOR_FOLIO,
    )).status).toBe(503);
  });
});
