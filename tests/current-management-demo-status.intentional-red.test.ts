...
test("Order 440 status records fiscal and hotel work without claiming Phase 7 completion", () => {
  ...
  expect(currentDelivery?.summary).toMatch(/durable fiscal submission.*authenticated provider.*immutable signed receipts.*merged through PR91 at 3503b0c/i);
  expect(currentDelivery?.summary).toContain("native ARM64");
  expect(currentDelivery?.summary).toContain("full upgrade compatibility");
  expect(currentDelivery?.summary).toContain("All six CI jobs and normal CodeQL passed");
  expect(currentDelivery?.summary).toContain("81 migrations / 128 public tables");
  expect(currentDelivery?.summary).toContain("post-merge schema/referee 11/11");
  expect(currentDelivery?.remaining).toContain("Merged main81 is not the preserved local77");
  expect(currentDelivery?.remaining).toContain("full workflow and independent integration proofs remain required");
  ...
  expect(currentDelivery?.summary).toMatch(/Astra Ultra RMS.*documented research without algorithm runtime or measured uplift/i);
