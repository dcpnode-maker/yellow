...
describe("Order 294 ordinary CGST section 13(2) accommodation time of supply", () => {
  ...
  test("accepts either payment evidence ordering and preserves the earlier receipt date", async () => {
    const service = new IndiaGstAccommodationTimeOfSupplyService();
    const booksEarlier = await service.resolve(tx([row({ books_date: "2043-06-14", bank_date: "2043-06-16", payment_date: "2043-06-14" })]), input({ paymentReceiptDate: "2043-06-14" }));
    expect(booksEarlier.paymentReceiptDate).toBe("2043-06-14");
    const bankEarlier = await service.resolve(tx([row({ books_date: "2043-06-16", bank_date: "2043-06-14", payment_date: "2043-06-14" })]), input({ paymentReceiptDate: "2043-06-14" }));
    expect(bankEarlier.paymentReceiptDate).toBe("2043-06-14");
    const equal = await service.resolve(tx([row({ books_date: "2043-06-15", bank_date: "2043-06-15", payment_date: "2043-06-15" })]), input());
    expect(equal.paymentReceiptDate).toBe("2043-06-15");
  });
