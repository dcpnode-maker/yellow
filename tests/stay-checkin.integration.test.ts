...
databaseDescribe("Order 200 governed arrival readiness and check-in", () => {
  test("P1: exact inspected due-in atomically becomes in-house with one fact/outbox and exact replay", async () => {
  ...
  test("P2/P4: identity and supervisor-inspection gates fail closed and return no PII", async () => {
    ...
    const cleanButUninspected = await checkIns!.getReadiness({
      tenantId: TENANT, propertyNode: GATED_PROPERTY, reservationId: GATED_RESERVATION,
      dirtyRoomOverrideAuthorized: false,
    });
    expect(cleanButUninspected).toMatchObject({
      canCheckIn: false,
      roomCondition: "clean",
      identityGate: { required: true, satisfied: true },
    });
    expect(cleanButUninspected.blockers).toEqual(["room_inspection_required"]);
    await expect(checkIns!.checkIn(command(GATED_RESERVATION, GATED_PROPERTY)))
      .rejects.toBeInstanceOf(CheckInConflictError);
    expect((await snapshot(GATED_RESERVATION)).facts).toBe(0);
    await deploy!`UPDATE unit_condition
      SET condition='inspected', updated_at=clock_timestamp(), updated_by=${ACTOR}::uuid
      WHERE tenant_id=${TENANT}::uuid AND space_id=${GATED_SPACE}::uuid`;
