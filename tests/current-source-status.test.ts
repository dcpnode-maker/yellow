...
test("Order469 distinguishes accepted source and timestamped delivery from current runtime", () => {
  expect(PROJECT_BUILD_SNAPSHOT.recordedAt).toBe("2026-09-13");
  expect(PROJECT_BUILD_SNAPSHOT.roadmap).toEqual({ phaseCount: 18, latestBuiltOrder: 469, currentOrder: 460, activePhase: 7 });
  ...
  expect(newer.map(({ order, state }) => [order, state])).toEqual([
    ...
    [467, "built_unverified"], [468, "independently_approved"], [469, "independently_approved"],
  ]);
