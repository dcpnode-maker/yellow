...
const groups = {
  ...
  revenue: [["nav-inventory", "inventory"], ["nav-restrictions", "restrictions"], ["nav-rates", "rates"], ["nav-market", "market"]],
...
test("Order459 groups the current 16 destinations exactly once in five native disclosures", () => {
  ...
  expect(html).toContain('<button class="domain-tab" id="nav-market" type="button" data-view="market" aria-controls="market-view"><svg class="domain-icon" aria-hidden="true" viewBox="0 0 256 256"><use href="#ph-chart-line-up"/></svg><span>Market evidence</span></button>');
