...
describe("Order 212 exact operator reservation travel adapter", () => {
  test("P0/P4: createApp binds one exact PUT route to the governed handler", () => {
    ...
    expect(server).toContain(
      "vehicleRegister, reservationTravel, pickupTaskDispatch, arrivalRoomCleaning, housekeepingDiscrepancies, vehicleParking, undefined, undefined, businessDayCarry, businessDaySeal, ownerTrustExpenses, {\n      submissions: fiscalSubmissions,\n      adapters: fiscalSubmissionAdapters,\n    }, reservationAlerts, new OperatingPerformanceService())",
    );
  });
