...
describe("Order 099 operator reservation booking workbench", () => {
  ...
  test("P4: browser sends canonical commands and renders only server confirmation truth", () => {
    const start = script.indexOf("  function renderReservationBookingOffers(options, issues) {");
    const end = script.indexOf("\n reservationSegmentLookupForm.addEventListener", start);
    expect(start).toBeGreaterThan(-1);
    expect(end).toBeGreaterThan(start);
    const bookingScript = script.slice(start, end);
    expect(bookingScript).toContain("`${fromValue}Z`");
    expect(bookingScript).toContain("`${toValue}Z`");
    expect(bookingScript).toContain("commit_arbitration_required");
    expect(bookingScript).toContain("result.reservation.confirmationNo");
    expect(bookingScript).toContain("result.reservation.status");
    expect(bookingScript).toContain("searchGeneration !== reservationBookingSearchGeneration");
    expect(bookingScript).toContain("property !== propertySelect.value");
    expect(bookingScript).not.toMatch(/Math\.round|\.toFixed|parseFloat/);
    // Device-local Overwatch transcript/preferences are permitted elsewhere in the
    // operator shell; the governed booking path itself must never persist a draft.
    expect(bookingScript).not.toMatch(/localStorage|sessionStorage|document\.cookie/);
  });
