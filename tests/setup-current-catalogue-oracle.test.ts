...
describe("Order 371 current setup catalogue oracle", () => {
  test("derives the migration and public-table frontier before checking setup", async () => {
    ...
    expect({ migrationCount: migrations.length, highestMigration, publicBaseTables }).toEqual({
      migrationCount: 77,
      highestMigration: 77,
      publicBaseTables: 127,
    });
    expect(setup).toContain("[ \"$tables\" = '127' ]");
    expect(setup).toContain("expected 127 after migrations 1-77");
    expect(setup).toContain("yellow_test tables: 127 after migrations 1-77");
