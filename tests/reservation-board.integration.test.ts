...
function row(id: string) {
  return {
    id, confirmation_no: `Y-${id.slice(-3)}`, status: "reserved", operational_state: "reserved",
...
describe("Order 166 bounded reservation board", () => {
  ...
  test("tied timestamps use the UUID tie-breaker and emit one canonical opaque cursor", async () => {
    ...
    expect(sql).toContain("WITH property_context AS MATERIALIZED");
    expect(sql).toContain("page_reservations AS MATERIALIZED");
    expect(sql).toContain("transaction_timestamp() AT TIME ZONE property.timezone");
    expect(sql).toContain("fact_type = 'reservation.checked_in'");
    expect(sql).toContain("fact_type = 'reservation.checked_out'");
    expect(sql).toContain("checkin_successor.supersedes = checkin_fact.id");
    expect(sql).toContain("checkout_successor.supersedes = checkout_fact.id");
    expect(sql).toContain("contradictory_checkout.fact_type = 'reservation.checked_out'");
    expect(sql).toContain("transaction_timestamp() AS as_of");
    expect(sql).toContain("summary.stay_to_instant > property_context.as_of");
  ...
  test("operational stay states are exact, separate from stored state and fail closed", async () => {
    const service = new ReservationBoardService();
    const list = (status: string, operationalState: string) => service.list((() => Promise.resolve([
      { ...row(RESERVATION_A), status, operational_state: operationalState },
    ])) as unknown as Tx, input());
    expect((await list("due_in", "due_in")).reservations[0]).toMatchObject({ status: "due_in", operationalState: "due_in" });
    expect((await list("in_house", "checked_in_today")).reservations[0]).toMatchObject({ status: "in_house", operationalState: "checked_in_today" });
    expect((await list("in_house", "stayover")).reservations[0]).toMatchObject({ status: "in_house", operationalState: "stayover" });
    expect((await list("due_out", "due_out")).reservations[0]).toMatchObject({ status: "due_out", operationalState: "due_out" });
    expect((await list("checked_out", "checked_out_today")).reservations[0]).toMatchObject({ status: "checked_out", operationalState: "checked_out_today" });
    expect((await list("checked_out", "checked_out")).reservations[0]).toMatchObject({ status: "checked_out", operationalState: "checked_out" });
    await expect(list("in_house", "arrived_maybe")).rejects.toBeInstanceOf(ReservationBoardConflictError);
  });
