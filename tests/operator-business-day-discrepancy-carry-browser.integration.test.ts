...
async function chromium(html: string, width: number, theme: string) {
  ...
  try {
    const proc = Bun.spawn([browser, "--headless=new", "--disable-gpu", "--no-sandbox", "--allow-file-access-from-files", `--window-size=${width},900`, "--virtual-time-budget=3000", "--dump-dom", file], { stdout: "pipe", stderr: "ignore" });
