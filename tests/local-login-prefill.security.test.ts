...
describe("Order194 local sign-in prefill", () => {
  ...
  test("automatic synthetic-demo entry uses only process-scoped credentials", async () => {
    const previous = Bun.env.YELLOW_PUBLIC_DEMO_AUTOMATIC_LOGIN;
    Bun.env.YELLOW_PUBLIC_DEMO_AUTOMATIC_LOGIN = "1";
    const received: unknown[] = [];
    try {
      const app = createApp({
        operatorApi: {
          publicReadOnlyDemoLogin: async (_request: Request, credentials: unknown) => {
            received.push(credentials);
            return new Response(JSON.stringify({ accessToken: "synthetic-token" }), {
              headers: { "content-type": "application/json" },
            });
          },
        } as never,
        operatorLocalReviewCredentials: {
          tenant: "yellow-demo",
          email: "operator@yellow.local",
          password: "process-only-secret",
        },
      });
      const response = await app.handle(new Request("http://yellow.test/api/v1/auth/demo:enter", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: "{}",
      }));
      expect(response.status).toBe(200);
      expect(await response.json()).toEqual({ accessToken: "synthetic-token" });
      ...
      if (previous === undefined) delete Bun.env.YELLOW_PUBLIC_DEMO_AUTOMATIC_LOGIN;
      else Bun.env.YELLOW_PUBLIC_DEMO_AUTOMATIC_LOGIN = previous;
    }
  });
