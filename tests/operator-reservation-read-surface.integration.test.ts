...
describe("Order 166 operator reservation read surface", () => {
  ...
  test("Order 206 minimizes arrival travel on the existing board transport", () => {
    ...
    expect(projection).not.toMatch(/pickupTaskId|taskStatus|taskState|travelId|notes|\bpartyId\b|contact|parking|vehicle/i);
  ...
  test("Order 207 minimizes departure travel on the existing board transport", () => {
    ...
    expect(projection).not.toMatch(/pickupRequested|pickupTaskLinked|pickupTaskId|taskStatus|taskState|travelId|notes|\bpartyId\b|contact|parking|vehicle/i);
    expect(operator).toContain("canonicalJson(reservationBoardJson(page))");
  });
