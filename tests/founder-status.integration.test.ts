...
describe("Order 064 recorded build snapshot", () => {
  test("P3: runtime snapshot is exact to the committed Gate-3 manifest", async () => {
    ...
    expect(PROJECT_BUILD_SNAPSHOT.roadmap.latestBuiltOrder).toBe(256);
    expect(PROJECT_BUILD_SNAPSHOT.review.gate3Debt).toBe(0);
    expect(PROJECT_BUILD_SNAPSHOT.review.state).toBe("built_unverified");
    expect(PROJECT_BUILD_SNAPSHOT.roadmap.currentOrder).toBe(257);
    ...
    expect(PROJECT_BUILD_SNAPSHOT.recordedWork.filter(({ state }) => state === "built_unverified")
      .map(({ order }) => Number(order))).toEqual([199, 236, 256]);
    ...
    const order256: { readonly state: string; readonly summary: string; readonly remaining?: string } | undefined =
      PROJECT_BUILD_SNAPSHOT.recordedWork.find(({ order }) => order === 256);
    expect(order256?.state).toBe("built_unverified");
    expect(order256?.summary).toMatch(/237–256/);
    expect(order256?.summary).toMatch(/pure tax evaluation/i);
    expect(order256?.summary).toMatch(/jurisdiction resolution/i);
    expect(order256?.summary).toMatch(/quote preview/i);
    expect(order256?.summary).toMatch(/canonical positive attribution snapshot/i);
    expect(order256?.summary).toMatch(/append-only persistence/i);
    expect(order256?.summary).toMatch(/occupancy definer-path repair/i);
    expect(order256?.summary).toMatch(/authoritative quoted-tax cart-hold binding/i);
    expect(order256?.summary).toMatch(/canonical positive posting topology/i);
    expect(order256?.summary).toMatch(/authoritative quoted-tax hold-to-reservation\/first-segment lineage/i);
    expect(order256?.summary).toMatch(/independently approved primary-folio eligibility read\/lock\/recheck prerequisite/i);
    expect(order256?.remaining).toMatch(/configured semantic routing/i);
    expect(order256?.remaining).toMatch(/governed positive posting/i);
    expect(order256?.remaining).toMatch(/correction\/reversal/i);
    expect(order256?.remaining).toMatch(/fiscal documents\/IRP/i);
    expect(order256?.remaining).toMatch(/independent product review/i);
    expect(order256?.remaining).toMatch(/Phase-7 completion remain pending/i);
