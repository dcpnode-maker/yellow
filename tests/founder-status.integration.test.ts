...
describe("Order 064 recorded build snapshot", () => {
  test("P3: runtime snapshot is exact to the committed Gate-3 manifest", async () => {
    ...
    expect(PROJECT_BUILD_SNAPSHOT.label).toBe("Current-source receiving integration in progress; credit-note register source accepted");
    expect(PROJECT_BUILD_SNAPSHOT.recordedAt).toBe("2026-09-13");
    expect(PROJECT_BUILD_SNAPSHOT.roadmap.latestBuiltOrder).toBe(469);
    ...
    // Historical receipts remain byte-for-byte asserted. Current milestones are
    // separately checked in current-source-status.test.ts and the full ID list.
    expect(PROJECT_BUILD_SNAPSHOT.recordedWork.filter(({ order }) => order <= 444)).toEqual([
      {
        order: 126,
    ...
    expect(recordedOrders).toEqual([
      ...
      453, 454, 455, 458, 459, 460, 461, 462, 463, 464, 465, 466, 467, 468, 469,
    ]);
    ...
    expect(PROJECT_BUILD_SNAPSHOT.recordedWork.filter(({ order }) => order < 440).every(({ state }) => state === "independently_approved")).toBeTrue();
    ...
    expect(PROJECT_BUILD_SNAPSHOT.recordedWork.at(-1)?.order).toBe(469);
