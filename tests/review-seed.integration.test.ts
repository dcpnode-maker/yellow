  ...
  return admin<Array<Record<string, unknown>>>`
    ...
    WHERE vehicle.tenant_id=${SEED_TENANT.id}::uuid
      ...
      AND vehicle.id IN (${first.vehicleExamples.arrivalVehicleId}::uuid,
        ...
        ${first.vehicleExamples.parkingVehicleId}::uuid)
...
databaseDescribe("Order 046 reproducible local-review seed", () => {
  ...
  test("P1: provisions exact identities, six-room inventory and governed rate evidence", async () => {
    ...
    for (const id of [
      ...
      first.vehicleExamples.parkingVehicleId,
  ...
  test("P2: every inventory aggregate carries one fact and one outbox event", async () => {
    ...
    expect(rows).toEqual([
      ...
      { entity_type: "space", aggregates: 9, facts: 9, events: 9 },
  ...
  test("Order 171 P1/P4: provisions only canonical local-review financial configuration", async () => {
    ...
    expect(rows[0]).toEqual({
      ...
      guest_accounts: 5, folios: 7, journals: 0, postings: 0, payments: 0, documents: 0,
    });
  });
  ...
  test("Order 202 P7: provisions one isolated daily in-house eligible room without generating a sheet", async () => {
    ...
    const profile = await admin<Array<{ cadence: string; matches: number }>>`
      ...
      WHERE type='vertical_profile' AND key='hotel' AND status='active'
        -- Launch extensions become effective at the canonical seed transaction.
        -- The housekeeping generator uses that same transaction-time lookup.
  ...
  test("Order 203 P5: reuses the isolated in-house fixture with one settled zero-balance folio only", async () => {
    expect(await departureReadinessFixtureSnapshot()).toEqual([{
      ...
      account_name: "Devika Menon Ledger",
  ...
  test("Order 205 P5: provisions three literal property vehicle rows without parking or lifecycle effects", async () => {
    expect(await vehicleFixtureSnapshot()).toEqual([
      ...
      {
        id: first.vehicleExamples.parkingVehicleId,
        reg_no: "PARK-YLW-01",
        make: "Mahindra",
        model: "XUV400",
        colour: "Everest White",
        driver_name: "Arjun Shah",
    ...
    const guards = await admin<Array<{
      ...
      SELECT
        (SELECT count(*)::int FROM fact_log WHERE entity_id IN (
          ...
          ${first.vehicleExamples.departureVehicleId}::uuid,
          ${first.vehicleExamples.parkingVehicleId}::uuid)) AS facts,
        (SELECT count(*)::int FROM outbox WHERE aggregate_id IN (
          ${first.vehicleExamples.arrivalVehicleId}::uuid,
          ${first.vehicleExamples.departureVehicleId}::uuid,
          ${first.vehicleExamples.parkingVehicleId}::uuid)) AS events,
        (SELECT count(*)::int FROM space_occupancy WHERE slot_ref IN (
          ${first.vehicleExamples.arrivalVehicleId}::uuid,
          ${first.vehicleExamples.departureVehicleId}::uuid,
          ${first.vehicleExamples.parkingVehicleId}::uuid)) AS occupancy_references,
        (SELECT count(*)::int FROM api_idempotency
          WHERE tenant_id=${SEED_TENANT.id}::uuid) AS idempotency_records
    `;
  ...
  test("Order 236 P6: provisions exact active parking choices and one current assignable vehicle", async () => {
    const parking = await admin<Array<{
      code: string; profile_key: string; capacity: number; status: string;
      assignments: number; claims: number;
    }>>`
      SELECT space.code, space.profile_key, space.capacity, space.status,
             count(vehicle.id)::int AS assignments,
             count(occupancy.id)::int AS claims
        FROM space
        LEFT JOIN vehicle ON vehicle.tenant_id=space.tenant_id
          AND vehicle.parking_space=space.id
        LEFT JOIN space_occupancy AS occupancy ON occupancy.tenant_id=space.tenant_id
          AND occupancy.space_id=space.id AND occupancy.period @> transaction_timestamp()
       WHERE space.tenant_id=${SEED_TENANT.id}::uuid
         AND space.property_node=${SEED_PROPERTY.id}::uuid
    ...
    expect(parking).toEqual([
      { code: "P-01", profile_key: "parking", capacity: 1, status: "active", assignments: 0, claims: 0 },
      { code: "P-02", profile_key: "parking", capacity: 1, status: "active", assignments: 0, claims: 0 },
      { code: "P-03", profile_key: "parking", capacity: 1, status: "active", assignments: 0, claims: 0 },
    ]);
    const vehicle = await admin<Array<{
      id: string; entered: boolean; exited_at: string | null; parking_space: string | null;
      reservation_status: string; current_segments: number;
    }>>`
      SELECT vehicle.id, vehicle.entered_at IS NOT NULL AS entered,
             vehicle.exited_at::text, vehicle.parking_space,
             reservation.status AS reservation_status,
             count(segment.id) FILTER (
               WHERE segment.status='in_house' AND segment.period @> transaction_timestamp()
             )::int AS current_segments
        FROM vehicle
        JOIN reservation ON reservation.tenant_id=vehicle.tenant_id
          AND reservation.id=vehicle.reservation_id
        LEFT JOIN reservation_segment AS segment ON segment.tenant_id=reservation.tenant_id
          AND segment.reservation_id=reservation.id
       WHERE vehicle.tenant_id=${SEED_TENANT.id}::uuid
         AND vehicle.property_node=${SEED_PROPERTY.id}::uuid
         AND vehicle.id=${first.vehicleExamples.parkingVehicleId}::uuid
       GROUP BY vehicle.id, reservation.status
    `;
    expect(vehicle).toEqual([{ id: first.vehicleExamples.parkingVehicleId, entered: true,
      exited_at: null, parking_space: null, reservation_status: "in_house", current_segments: 1 }]);
    const billingStay = await admin<Array<{
      primary_guests: number; exact_occupancy: number; primary_folios: number;
      open_primary_folios: number; account_matches: number; posting_lines: number;
    }>>`
      SELECT count(DISTINCT guest.party_id) FILTER (WHERE guest.role='primary')::int AS primary_guests,
             count(DISTINCT occupancy.id) FILTER (WHERE occupancy.period=segment.period
               AND occupancy.slot_kind='segment' AND occupancy.exclusive AND occupancy.claim='[0,)'::int4range)::int
               AS exact_occupancy,
             count(DISTINCT folio.id) FILTER (WHERE folio.window_no=1)::int AS primary_folios,
             count(DISTINCT folio.id) FILTER (WHERE folio.window_no=1 AND folio.status='open')::int
               AS open_primary_folios,
             count(DISTINCT folio.id) FILTER (WHERE folio.account_id=${first.housekeepingExamples.departureAccountId}::uuid)::int
               AS account_matches,
             COALESCE(sum(balance.lines),0)::int AS posting_lines
        FROM vehicle
        JOIN reservation ON reservation.tenant_id=vehicle.tenant_id
          AND reservation.id=vehicle.reservation_id
        JOIN reservation_segment AS segment ON segment.tenant_id=reservation.tenant_id
          AND segment.reservation_id=reservation.id AND segment.status='in_house'
        LEFT JOIN reservation_guest AS guest ON guest.tenant_id=reservation.tenant_id
          AND guest.reservation_id=reservation.id
        LEFT JOIN space_occupancy AS occupancy ON occupancy.tenant_id=segment.tenant_id
          AND occupancy.slot_ref=segment.id
        LEFT JOIN folio ON folio.tenant_id=reservation.tenant_id
          AND folio.reservation_id=reservation.id
        LEFT JOIN folio_balance AS balance ON balance.tenant_id=folio.tenant_id
          AND balance.folio_id=folio.id
       WHERE vehicle.tenant_id=${SEED_TENANT.id}::uuid
         AND vehicle.id=${first.vehicleExamples.parkingVehicleId}::uuid
       GROUP BY vehicle.id
    `;
    expect(billingStay).toEqual([{ primary_guests: 1, exact_occupancy: 1, primary_folios: 1,
      open_primary_folios: 1, account_matches: 1, posting_lines: 0 }]);
  });
  ...
  test("P3: identical rerun is an exact no-op", async () => {
    ...
    expect(logLines).toContain(
      `review vehicle fixtures: arrival=${first.vehicleExamples.arrivalVehicleId} departure=${first.vehicleExamples.departureVehicleId} parking=${first.vehicleExamples.parkingVehicleId}`,
    );
    expect(logLines).toContain(
  ...
  test("P5: login is least-scope and availability returns six real options", async () => {
    ...
    const approverClaims = await tokens.verify(approverLoginBody.accessToken);
    expect(approverClaims).toMatchObject({
      sub: first.approverUserId,
      tid: SEED_TENANT.id,
      scp: "crm.parties:read crm.parties:write financials.adjustments:post-seal financials.adjustments:write financials.business-day:approve-discrepancy-carry financials.business-days:read financials.cashiers:operate financials.cashiers:read financials.cashiers:supervise financials.charges:write financials.folios:close financials.folios:open financials.folios:read financials.folios:settle financials.receivables:approve financials.receivables:read financials.receivables:transfer financials.transfers:write financials.trust:approve-negative financials.trust:post housekeeping.arrival-tasks:create housekeeping.arrival-tasks:read housekeeping.conditions:initialize housekeeping.discrepancies:read housekeeping.discrepancies:report housekeeping.sheets:generate housekeeping.sheets:read housekeeping.tasks:inspect housekeeping.tasks:read housekeeping.tasks:work inventory.availability:read inventory.blocks:read inventory.blocks:write inventory.configuration:read inventory.configuration:write inventory.holds:read inventory.holds:write inventory.offline_leases:read inventory.offline_leases:write inventory.policy:read inventory.policy:write inventory.restriction:read inventory.restriction:write rates.configuration:read rates.configuration:write rates.pricing:read rates.pricing:write reservations.booking:write reservations.guests:read reservations.guests:write reservations.lifecycle:read reservations.lifecycle:write reservations.segments:read reservations.segments:write stay-operations.checkin:commit stay-operations.checkin:dirty-room-override stay-operations.checkin:read stay-operations.checkout:commit stay-operations.checkout:read stay-operations.departure-services:confirm stay-operations.departure-services:dispatch stay-operations.departure-services:escalate stay-operations.departure-services:read stay-operations.departure-services:request stay-operations.departure-services:work stay-operations.pickup-tasks:dispatch stay-operations.pickup-tasks:work stay-operations.vehicles:park stay-operations.vehicles:read tax-fiscal.india-valuation:finalize",
    });

    expect(approverClaims?.scp.split(" ")).not.toContain("financials.business-days:seal");
    ...
    expect(body.options.map(({ sellableUnitName, bookable }) => ({ sellableUnitName, bookable }))).toEqual([
      { sellableUnitName: "Room 101", bookable: true },
      { sellableUnitName: "Room 102", bookable: true },
      { sellableUnitName: "Room 103", bookable: true },
      { sellableUnitName: "Room 201", bookable: true },
      { sellableUnitName: "Room 202", bookable: true },
      // The synthetic cashier stay is genuinely in-house in Room 203. Its
      // occupancy is deliberate proof that the demo cannot sell the same room.
      { sellableUnitName: "Room 203", bookable: false },
    ]);
  });
