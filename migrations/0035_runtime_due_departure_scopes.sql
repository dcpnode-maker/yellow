-- Order 233: expose only bounded tenant/property discovery for reservations
-- whose database-derived property-local calendar date is their coherent latest
-- departure date. The runtime worker receives no reservation or segment details.

CREATE FUNCTION public.runtime_due_departure_scopes(p_limit integer)
RETURNS TABLE(tenant_id uuid, property_node uuid)
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
SET search_path = pg_catalog, public, pg_temp
AS $runtime_due_departure_scopes$
BEGIN
  ...
  SELECT reservation.tenant_id, reservation.property_node
   ...
   LIMIT p_limit;
END
$runtime_due_departure_scopes$;

ALTER FUNCTION public.runtime_due_departure_scopes(integer) OWNER TO yellow_owner;
REVOKE ALL ON FUNCTION public.runtime_due_departure_scopes(integer)
  FROM PUBLIC, app_role, yellow_runtime;
GRANT EXECUTE ON FUNCTION public.runtime_due_departure_scopes(integer) TO yellow_runtime;
