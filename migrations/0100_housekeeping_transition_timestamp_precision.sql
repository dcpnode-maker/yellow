-- Order 635: align governed housekeeping transition evidence with the
-- millisecond timestamp precision exposed by the TypeScript/API boundary.
-- Status, condition, tenant, property, actor and same-transaction effects remain unchanged.

CREATE OR REPLACE FUNCTION public.transition_housekeeping_task(
  p_tenant uuid,
  p_property uuid,
  p_task uuid,
  p_action text,
