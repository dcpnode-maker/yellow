-- Order 286: exact, read-only India GST supplier SEZ-status evidence.
...
CREATE TABLE public.india_gst_supplier_sez_status (
  ...
  CONSTRAINT india_gst_supplier_sez_status_pk PRIMARY KEY (tenant_id, id),
  CONSTRAINT india_gst_supplier_sez_status_identity_uq UNIQUE (
    tenant_id,
    supplier_registration_id,
    supplier_registration_evidence_hash,
    status_as_of
  ),
  CONSTRAINT india_gst_supplier_sez_status_registration_fk
    FOREIGN KEY (tenant_id, supplier_registration_id)
    REFERENCES public.property_fiscal_registration (tenant_id, id),
  CONSTRAINT india_gst_supplier_sez_status_supplier_hash_ck CHECK (
    supplier_registration_evidence_hash ~ '^[0-9a-f]{64}$'
  ),
  CONSTRAINT india_gst_supplier_sez_status_registration_status_ck CHECK (
    gst_registration_status = 'active'
  ),
  CONSTRAINT india_gst_supplier_sez_status_taxpayer_type_ck CHECK (
    gst_taxpayer_type IN ('regular', 'sez_unit', 'sez_developer')
  ),
  CONSTRAINT india_gst_supplier_sez_status_source_ck CHECK (
    gst_status_source = 'gst_common_portal'
  ),
  CONSTRAINT india_gst_supplier_sez_status_status_hash_ck CHECK (
    gst_status_evidence_sha256 ~ '^[0-9a-f]{64}$'
  ),
  CONSTRAINT india_gst_supplier_sez_status_approval_shape_ck CHECK (
    (
  ...
  CONSTRAINT india_gst_supplier_sez_status_legal_rule_ck CHECK (
    legal_rule = 'IGST_ACT_7_5_B_AND_8_2_SUPPLIER_STATUS'
  )
);

ALTER TABLE public.india_gst_supplier_sez_status ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.india_gst_supplier_sez_status FORCE ROW LEVEL SECURITY;
CREATE POLICY tenant_isolation ON public.india_gst_supplier_sez_status
  USING (
    tenant_id = NULLIF(
...
ALTER TABLE public.india_gst_supplier_sez_status OWNER TO yellow_owner;
REVOKE ALL ON TABLE public.india_gst_supplier_sez_status FROM PUBLIC, app_role;
REVOKE ALL ON TABLE public.india_gst_supplier_sez_status FROM yellow_runtime;
